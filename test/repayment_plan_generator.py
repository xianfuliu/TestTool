...
class RepaymentPlanGenerator:
    ...
    def generate_plan(self, loan_amount, installment_count, 
                     loan_apply_no,
                     start_date='20250227', 
                     output_file=None,
        ...
        # 生成还款计划
        plan_df = self._generate_repayment_plan(loan_amount, installment_count, 
                                               start_date, loan_apply_no)
        ...
        if generate_loan_file:
            ...
            loan_file = self.generate_loan_file(loan_apply_no, cap_loan_no, 
                                              loan_amount, installment_count, 
                                              start_date)
    ...
    def _generate_repayment_plan(self, loan_amount, installment_count, 
                                start_date, loan_apply_no):
        """生成还款计划的核心逻辑"""
        # 获取样板数据
        ...
        if 'avg_principal_ratios' in self.period_stats[installment_count]:
            principal_ratios = self.period_stats[installment_count]['avg_principal_ratios']
        else:
        ...
        random_part = ''.join(random.choices('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ', k=20))
        ...
        # 计算费用
        regular_fee = round(loan_amount * regular_fee_structure.get('fee_rate', 0))  # 四舍五入
        regular_other_fee = round(loan_amount * regular_fee_structure.get('other_fee_rate', 0))  # 四舍五入
        last_fee = round(loan_amount * last_fee_structure.get('fee_rate', 0))  # 四舍五入
        last_other_fee = round(loan_amount * last_fee_structure.get('other_fee_rate', 0))  # 四舍五入
        ...
        for i in range(1, installment_count + 1):
            # 本金比例
            principal_ratio = principal_ratios[i-1] if i-1 < len(principal_ratios) else 0
            ...
            # 利息计算
            template_interest_rate = template_df[template_df['installCnt'] == i]['totalInterest'].iloc[0] / template_analysis['loan_amt']
    ...
    def generate_loan_file(self, loan_apply_no, cap_loan_no, loan_amt, install_cnt, 
                          pay_date, output_file=None):
        """
        生成借款文件
        ...
        loan_header = [
            "loanApplyNo", "capLoanNo", "transDate", "payDate", "transAmt", 
            "totalCnt", "yearRate", "capRate", "assetSide", "capCode"
        ]
        
        loan_data = [
            loan_apply_no, cap_loan_no, pay_date, pay_date, str(loan_amt),
            str(install_cnt), year_rate, cap_rate, asset_side, cap_code
        ]
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for config in configs:
                if 'cap_loan_no' in config:
                    ...
                    data_row = [
                        config['loan_apply_no'], config['cap_loan_no'], pay_date, 
                        pay_date, str(config['loan_amount']), str(config['installment_count']),
                        year_rate, cap_rate, asset_side, cap_code
                    ]
    ...
    def _analyze_single_loan(self, loan_df):
        """分析单个借据的还款计划"""
        try:
            loan_df['installCnt'] = pd.to_numeric(loan_df['installCnt'], errors='coerce')
            
            loan_amt = float(loan_df['loanAmt'].iloc[0])
            installment_cnt = int(loan_df['installCnt'].max())
            
            numeric_cols = ['totalAmt', 'totalPrincipal', 'totalFee', 
                           'totalInterest', 'totalOtherFee']
    ...
    def _analyze_fee_structure(self, loan_df, loan_amt):
        ...
        regular_periods = loan_df[loan_df['installCnt'] < loan_df['installCnt'].max()].copy()
        if not regular_periods.empty:
            regular_periods.loc[:, 'fee_rate'] = regular_periods['totalFee'] / loan_amt
            regular_periods.loc[:, 'other_fee_rate'] = regular_periods['totalOtherFee'] / loan_amt
            regular_periods.loc[:, 'total_fee_rate'] = regular_periods['fee_rate'] + regular_periods['other_fee_rate']
    ...
    def _calculate_apr(self, loan_df, loan_amt):
        ...
        try:
            ...
            try:
                monthly_irr = newton(lambda r: npv(r, cashflows), 0.01, maxiter=100, tol=1e-10)
            except:
                low, high = -0.99, 10.0
    ...
    def _calculate_period_statistics(self):
        ...
        for period, loans in self.period_analysis.items():
            if loans:
                ...
                self.period_summary[period] = self._calculate_average_fee_structure(fee_structures)
                
                if principal_ratios_list:
                    avg_principal_ratios = np.mean(principal_ratios_list, axis=0)
                    self.period_stats[period]['avg_principal_ratios'] = avg_principal_ratios.tolist()
    ...
    def show_period_summary(self):
        ...
        print("\n" + "=" * 100)
        print(f"{'期数':>6} {'样本数':>8} {'平均年化利率':>12} {'最低年化':>10} {'最高年化':>10} {'平均借款金额':>12}")
        print("-" * 100)
        
        sorted_periods = sorted(self.period_stats.keys())
        for period in sorted_periods:
            stats = self.period_stats[period]
            print(f"{period:6d} {stats['sample_count']:8d} {stats['avg_apr']:12.2f}% "
...
def main():
    """使用示例"""
    # 初始化生成器（自动加载模板文件）
    generator = RepaymentPlanGenerator(r"C:\Users\admin\Desktop\哈喽催收管理\对账文件造数\20250227\plan_20250227.txt")
