...
class ApiTemplateDialog(QDialog):
    ...
    def __init__(
        self, parent=None, template_data=None, project_id=None, folder_id=None
    ):
        super().__init__(parent)
...
class ApiFolderDialog(QDialog):
    ...
    def validate_and_accept(self):
        ...
        if self.folder_service.check_folder_name_exists(
            project_id, parent_id, folder_name, exclude_folder_id
        ):
            Toast.warning(
                self,
                "名称重复",
                f"当前目录下已存在名为 '{folder_name}' 的文件夹，请使用其他名称！",
            )
            self.name_edit.setFocus()
...
class ApiTemplateManager(QWidget):
    ...
    def init_ui(self):
        ...
        self.tree_widget.item_dragged_with_position.connect(
            self.on_item_dragged_with_position
        )
    ...
    def load_api_tree(self, preserve_expanded_state=True, additional_expanded_ids=None):
        ...
        """
        print(
            f"load_api_tree调用：preserve_expanded_state={preserve_expanded_state}, additional_expanded_ids={additional_expanded_ids}"
        )
    ...
    def restore_expanded_state(self, folder_map, expanded_folder_ids):
        """恢复之前展开的文件夹状态"""
        print(
            f"restore_expanded_state调用：folder_map大小={len(folder_map)}, expanded_folder_ids={expanded_folder_ids}"
        )
        ...
        def find_folder_item(item, target_folder_id):
            ...
            if (
                data
                and data["type"] == "folder"
                and data["data"]["id"] == target_folder_id
            ):
    ...
    def on_blank_area_clicked(self):
        ...
        # 如果没有打开的标签页，显示提示信息
        if (
            hasattr(self, "tabbed_editor")
            and self.tabbed_editor
            and self.tabbed_editor.tab_widget.count() == 0
        ):
    ...
    def on_tree_item_clicked(self, item):
        ...
        if item_type == "folder":
            ...
            if "id" in item_data and item_data["id"]:
                try:
                    latest_template_data = self.api_service.get_template_by_id(
                        item_data["id"]
                    )
    ...
    def on_item_dragged(self, source_item, target_item):
        ...
        try:
            ...
            if source_template["folder_id"] != target_folder_id:
                self.api_service.update_template_folder(
                    source_template["id"], target_folder_id
                )
            ...
            self.load_api_tree(
                preserve_expanded_state=True,
                additional_expanded_ids=expanded_folder_ids,
            )
    ...
    def on_item_dragged_with_position(
        ...
        print(
            f"正在将 '{source_template['name']}' 移动到 '{target_template['name']}' 的{position_text}..."
        )
        ...
        try:
            ...
            # 根据位置重新计算并更新排序顺序
            self.update_template_order_with_position(
                source_template["id"],
                target_folder_id,
                target_template["id"],
                drop_position,
            ...
            # 显示成功消息
            print(
                f"成功将 '{source_template['name']}' 移动到 '{target_template['name']}' 的{position_text}"
            )

            # 记录操作历史（用于撤销）
            self._record_drag_operation(
                source_template["id"],
                original_folder_id,
                original_sort_order,
                target_folder_id,
            ...
            self._restore_original_state(
                source_template["id"],
                source_template["folder_id"],
                source_template.get("sort_order", 0),
            )

    def update_template_order_with_position(
        self, source_template_id, folder_id, target_template_id, drop_position
    ):
        """根据位置更新模板的排序顺序（优化版）"""
        try:
            # 获取目标文件夹中的所有模板（包含源模板，用于计算完整列表）
            if folder_id:
                all_templates_in_folder = self.api_service.get_templates_by_folder(
                    folder_id
                )
            else:
                # 对于根节点，只获取folder_id为None的模板
                all_templates = self.api_service.get_templates_by_project(
                    self.current_project
                )
                all_templates_in_folder = [
                    t for t in all_templates if t.get("folder_id") is None
                ]

            # 过滤掉源模板（如果已经在文件夹中）
            templates = [
                t for t in all_templates_in_folder if t["id"] != source_template_id
            ]
            ...
            if drop_position == "above":
                ...
                if target_index == 0:
                    ...
                    if target_sort - prev_sort > 1:
                        ...
                        # 间距不足，需要重新分配排序值
                        new_sort_order = self._recalculate_sort_order(
                            templates, target_index, "above"
                        )
    ...
    def _record_drag_operation(
        self, template_id, original_folder_id, original_sort_order, new_folder_id
    ):
        """记录拖动操作历史（用于撤销功能）"""
        try:
            # 初始化操作历史列表
    ...
    def _restore_original_state(
        self, template_id, original_folder_id, original_sort_order
    ):
        """尝试恢复原始状态（错误恢复）"""
        try:
            # 恢复文件夹ID
    ...
    def add_api_folder(self):
        ...
        if self.current_folder:
            ...
            if current_depth >= 3:
                Toast.warning(
                    self, "提示", "文件夹层级最多允许三级，无法继续创建子文件夹"
                )
                return
        else:
            # 如果没有选中文件夹，创建一级文件夹
            ...
            for tab_id, tab_data in self.tabbed_editor.tabs.items():
                ...
                if "id" in current_data and current_data["id"] == template_id:
                    ...
                    Toast.information(
                        self,
                        "模板位置已更新",
                        f"接口 '{latest_template['name']}' 的位置已更新，请保存以确认更改",
                    )
                    break
    ...
    def on_template_saved(self, template_data):
        ...
        try:
            ...
            if self.api_service.check_template_name_exists(
                project_id, folder_id, template_name, template_id
            ):
                Toast.warning(
                    self,
                    "名称重复",
                    f"当前目录下已存在名为 '{template_name}' 的接口模板，请使用不同的名称",
                )
                return False  # 保存失败
            ...
            if "id" in template_data:
                ...
                latest_template = self.api_service.get_template_by_id(
                    template_data["id"]
                )
                if latest_template:
                ...
                save_success = self.api_service.update_template(
                    template_data["id"], template_data
                )
    ...
    def show_tree_context_menu(self, position):
        ...
        if item:
                ...
                if data["type"] == "folder":
                    ...
                    if folder_depth < 3:
                        ...
                        add_folder_action.triggered.connect(
                            lambda: self.add_subfolder(data["data"])
                        )
                        menu.addAction(add_folder_action)

                    add_template_action = QAction("新增接口", self)
                    add_template_action.triggered.connect(
                        lambda: self.add_api_template_to_folder(data["data"])
                    )
                    menu.addAction(add_template_action)

                    # 添加粘贴菜单项
                    if (
                        hasattr(self, "copied_template_data")
                        and self.copied_template_data
                    ):
                        paste_action = QAction("粘贴", self)
                        paste_action.triggered.connect(
                            lambda: self.paste_with_edit(
                                {"type": "folder", "data": data["data"]}
                            )
                        )
                    ...
                    edit_action.triggered.connect(
                        lambda: self.edit_api_folder(data["data"])
                    )
                    menu.addAction(edit_action)

                    delete_action = QAction("删除", self)
                    delete_action.triggered.connect(
                        lambda: self.delete_api_folder(data["data"])
                    )
                    ...
                    copy_action.triggered.connect(
                        lambda: self.copy_api_template_to_edit(data["data"])
                    )
                    ...
                    delete_action = QAction("删除", self)
                    delete_action.triggered.connect(
                        lambda: self.delete_api_template(data["data"])
                    )
                    menu.addAction(delete_action)
        else:
            # 空白区域（根目录）的右键菜单
            if hasattr(self, "copied_template_data") and self.copied_template_data:
                paste_action = QAction("粘贴", self)
                paste_action.triggered.connect(
                    lambda: self.paste_with_edit({"type": "root", "data": None})
                )
                menu.addAction(paste_action)
    ...
    def copy_api_template_to_edit(self, template_data):
        ...
        # 生成新的副本名称
        try:
            template_copy["name"] = self.generate_copy_name(
                template_data["name"], template_data.get("folder_id")
            )
        except Exception as e:
            return
    ...
    def paste_with_edit(self, target_data):
        ...
        if target_data["type"] == "folder":
            ...
            try:
                if self.api_service.check_template_name_exists(
                    self.current_project, folder_data["id"], original_name
                ):
                    # 存在同名接口，提示用户
            ...
            try:
                ...
                print(
                    f"粘贴操作：获取完整模板数据，模板名称='{new_template.get('name') if new_template else 'None'}'"
                )
            ...
            try:
                if self.api_service.check_template_name_exists(
                    self.current_project, None, original_name
                ):
                    # 存在同名接口，提示用户
            ...
            try:
                ...
                print(
                    f"粘贴操作：准备创建模板，项目ID={self.current_project}, 文件夹ID=None"
                )
