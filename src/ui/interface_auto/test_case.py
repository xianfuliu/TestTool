...
class ApiTemplateTreeWidget(QTreeWidget):
    ...
    def startDrag(self, supported_actions):
        ...
        if self.parent_manager:
            icon = self.parent_manager.get_api_icon_by_method(
                template_data.get("method", "GET")
            )
...
class CaseFolderDialog(QDialog):
    ...
    def __init__(
        self, parent=None, folder_data=None, project_id=None, parent_folder_id=None
    ):
        super().__init__(parent)
    ...
    def accept(self):
        ...
        try:
            ...
            if folder_service.check_folder_name_exists(
                project_id, parent_id, folder_name, exclude_id
            ):
                Toast.warning(
                    self, "警告", f"同一级目录下已存在名为 '{folder_name}' 的文件夹！"
                )
                return
...
class DraggableCaseTreeWidget(QTreeWidget):
    ...
    item_dragged_with_position = pyqtSignal(
        QTreeWidgetItem, QTreeWidgetItem, QTreeWidgetItem, str
    )  # 带位置的拖拽信号
        ...
        try:
            ...
            if not target_item:
                ...
                if target_type == "folder":
                    if drop_position == "on":
                        # 拖拽到文件夹内部
                        self.handle_drop_to_folder(
                            drag_type, drag_id, target_data["data"]["id"]
                        )
                    else:
                        ...
                        if parent_item:
                            ...
                            self.handle_drop_to_folder_with_position(
                                drag_type,
                                drag_id,
                                parent_data["data"]["id"],
                                target_data["data"]["id"],
                            ...
                            self.handle_drop_to_root_with_position(
                                drag_type,
                                drag_id,
                                target_data["data"]["id"],
                                drop_position,
                    ...
                    if parent_item:
                        ...
                        self.handle_drop_to_root_with_position(
                            drag_type, drag_id, target_data["data"]["id"], drop_position
                        )
    ...
    def handle_drop_to_root_with_position(
        self, drag_type, drag_id, target_id, drop_position
    ):
        """处理带位置的拖拽到根节点"""
        if drag_type == "case":
            # 发出带位置的拖拽信号
            ...
            if source_item and target_item:
                ...
                self.item_dragged_with_position.emit(
                    source_item, target_item, target_item, drop_position
                )

    def handle_drop_to_folder_with_position(
        self, drag_type, drag_id, folder_id, target_id, drop_position
    ):
        """处理带位置的拖拽到文件夹"""
        if drag_type == "case":
            # 发出带位置的拖拽信号
            ...
            if source_item and folder_item and target_item:
                # 传递文件夹、目标项和位置信息
                self.item_dragged_with_position.emit(
                    source_item, folder_item, target_item, drop_position
                )
...
class TestCaseManager(QWidget):
    ...
    def init_ui(self):
        ...
        self.new_folder_btn.setStyleSheet(
            "QPushButton { background-color: transparent; border: none; } QPushButton:hover { background-color: #f0f0f0; }"
        )
        ...
        self.refresh_btn.setStyleSheet(
            "QPushButton { background-color: transparent; border: none; margin-left: 10px; } QPushButton:hover { background-color: #f0f0f0; }"
        )
        ...
        self.case_tree.item_dragged_with_position.connect(
            self.on_case_dragged_with_position
        )
        ...
        # 连接接口模板编辑请求信号
        self.tabbed_case_editor.api_template_edit_requested.connect(
            self.api_template_edit_requested.emit
        )
    ...
    def create_new_folder(self):
        ...
        dialog = CaseFolderDialog(
            self, project_id=self.current_project, parent_folder_id=parent_folder_id
        )
    ...
    def get_expanded_case_folder_ids(self):
        ...
        self._collect_expanded_case_folder_ids(
            self.case_tree.invisibleRootItem(), expanded_folder_ids
        )
    ...
    def get_expanded_api_folder_ids(self):
        """获取当前展开的接口模板文件夹ID集合"""
        expanded_folder_ids = set()
        self._collect_expanded_api_folder_ids(
            self.api_tree.invisibleRootItem(), expanded_folder_ids
        )
        return expanded_folder_ids
    ...
    def load_case_tree(self, preserve_expanded_state=True):
        ...
        try:
            ...
            def add_folder_to_tree(folder_data, parent_item=None):
                ...
                folder_item.setData(
                    0, Qt.UserRole, {"type": "folder", "data": folder_data}
                )
    ...
    def load_api_templates(self, preserve_expanded_state=True):
        ...
        try:
            ...
            # 加载接口模板文件夹
            folders = self.api_folder_service.get_folders_by_project(
            ...
            for template in templates:
                ...
                template_item.setData(
                    0, Qt.UserRole, {"type": "template", "data": template}
                )
                template_item.setToolTip(
                    0,
                    f"{template['method']} {template['url_path']}\n{template.get('description', '')}",
                )

                # 设置接口模板图标（根据HTTP方法动态设置）
                template_item.setIcon(
                    0, self.get_api_icon_by_method(template.get("method"))
                )
    ...
    def filter_api_templates(self):
        ...
        # 保存搜索前的展开状态
        original_expanded_ids = (
            self.get_expanded_api_folder_ids() if not search_text else set()
        )
                ...
                for folder_id in matched_folder_ids:
                    if folder_id in folder_map:
                        ...
                        if folder and folder["parent_id"]:
                            ...
                            while parent_id:
                                parent_folder = next(
                                    (f for f in folders if f["id"] == parent_id), None
                                )
    ...
    def open_case_for_editing(self, case_data):
        ...
        try:
            ...
            # 调试信息：检查用例数据
            print(
                f"[DEBUG open_case_for_editing] 传入的case_data: id={case_data.get('id')}, name={case_data.get('name')}"
            )
            print(
                f"[DEBUG open_case_for_editing] 获取的完整数据: id={full_case_data.get('id')}, name={full_case_data.get('name')}"
            )
        ...
        try:
            ...
            if isinstance(case_data, str):
                ...
                if not full_case_data:
                    Toast.warning(
                        self, "错误", f"无法找到用例ID为 {case_data} 的测试用例"
                    )
                    return
                self.current_case_data = full_case_data
                ...
                # 使用tabbed_case_editor打开用例进行编辑
                self.tabbed_case_editor.open_case(
                    full_case_data, self.current_project, case_data.get("folder_id")
                )
            else:
    ...
    def on_case_dragged(self, source_item, target_item):
        ...
        try:
            ...
            if source_case["folder_id"] != target_folder_id:
                # 获取完整的用例数据，包括步骤信息
                full_case_data = self.case_service.get_case_with_steps(
                    source_case["id"]
                )
                if not full_case_data:
            ...
            if "NoneType" in str(e) or "folder_id" in str(e).lower():
                Toast.error(
                    self, "拖拽到根节点操作失败，请确保目标用例有有效的文件夹信息"
                )
            else:
                Toast.error(self, f"拖拽操作失败: {str(e)}")

    def on_case_dragged_with_position(
        self, source_item, folder_item, target_item, drop_position
    ):
        ...
        if not source_data or source_data["type"] != "case":
            print(
                f"[DEBUG] 源数据类型不正确: {source_data.get('type') if source_data else 'None'}"
            )
            return
        ...
        if not target_data or target_data["type"] != "case":
            print(
                f"[DEBUG] 目标数据类型不正确: {target_data.get('type') if target_data else 'None'}"
            )
            return
        ...
        print(
            f"正在将 '{source_case['name']}' 移动到 '{target_case['name']}' 的{position_text}..."
        )
        ...
        try:
            ...
            # 根据位置重新计算并更新排序顺序
            self.update_case_order_with_position(
                source_case["id"], target_folder_id, target_case["id"], drop_position
            )
            ...
            # 显示成功消息
            print(
                f"成功将 '{source_case['name']}' 移动到 '{target_case['name']}' 的{position_text}"
            )
            ...
            try:
                self._restore_case_original_state(
                    source_case["id"], original_folder_id, original_sort_order
                )
                print("已尝试恢复原始状态")
    ...
    def update_case_order(self, case_id, folder_id):
        ...
        try:
            # 获取目标文件夹中的所有用例
            cases = (
                self.case_service.get_cases_by_folder(self.current_project, folder_id)
                if folder_id
                else self.case_service.get_cases_by_project(self.current_project)
            )
    ...
    def update_case_order_with_position(
        self, case_id, folder_id, target_case_id, drop_position
    ):
        """根据位置更新测试用例的排序顺序（优化版，参考接口模板实现）"""
        print(
            f"[DEBUG] 开始更新用例顺序: case_id={case_id}, folder_id={folder_id}, target_case_id={target_case_id}, drop_position={drop_position}"
        )

        try:
            # 获取目标文件夹中的所有用例（包含源用例，用于计算完整列表）
            if folder_id:
                all_cases_in_folder = self.case_service.get_cases_by_folder(
                    self.current_project, folder_id
                )
            else:
                # 对于根节点，只获取folder_id为None的用例
                all_cases = self.case_service.get_cases_by_project(self.current_project)
                all_cases_in_folder = [
                    c for c in all_cases if c.get("folder_id") is None
                ]
            ...
            if drop_position == "above":
                ...
                if target_index == 0:
                    ...
                    if target_sort - prev_sort > 1:
                        ...
                        # 间距不足，需要重新分配排序值
                        new_sort_order = self._recalculate_case_sort_order(
                            cases, target_index, "above"
                        )
            ...
            print(
                f"[DEBUG] 用例排序更新成功: case_id={case_id}, new_sort_order={new_sort_order}"
            )
    ...
    def _check_case_sort_order_conflict(self, folder_id):
        ...
        try:
            if folder_id:
                cases = self.case_service.get_cases_by_folder(
                    self.current_project, folder_id
                )
            else:
                all_cases = self.case_service.get_cases_by_project(self.current_project)
    ...
    def _normalize_case_sort_orders(self, folder_id):
        ...
        try:
            ...
            for i, case in enumerate(sorted_cases):
                self.case_service.update_case_order(
                    case["id"], folder_id, (i + 1) * 100
                )

        except Exception as e:
            print(f"标准化用例排序值失败: {e}")

    def _restore_case_original_state(
        self, case_id, original_folder_id, original_sort_order
    ):
        """恢复用例的原始状态（用于错误恢复）"""
        try:
            # 恢复文件夹ID
            self.case_service.update_case_order(
                case_id, original_folder_id, original_sort_order
            )
    ...
    def on_case_saved(self, case_data):
        ...
        try:
            ...
            # 关键修复：通知标签页编辑器更新内部状态
            # 查找当前活动的标签页ID
            if hasattr(self, "tabbed_case_editor") and hasattr(
                self.tabbed_case_editor, "current_tab_id"
            ):
                ...
                if current_tab_id:
                    ...
                    if current_tab_id in self.tabbed_case_editor.tabs:
                        ...
                        print(
                            f"=== DEBUG: 已直接更新标签页状态 - tab_id: {current_tab_id}, case_id: {case_data.get('id')} ==="
                        )
    ...
    def update_editor_display(self):
        """根据标签页数量更新编辑器显示状态"""
        if hasattr(self, "tabbed_case_editor") and hasattr(
            self.tabbed_case_editor, "tab_widget"
        ):
    ...
    def create_test_case(self):
        ...
        # 打开新的标签页用于创建测试用例
        self.tabbed_case_editor.open_case(
            case_data=None, project_id=self.current_project, folder_id=folder_id
        )
    ...
    def create_test_case_in_folder(self, folder_data):
        ...
        # 打开新的标签页用于创建测试用例
        self.tabbed_case_editor.open_case(
            case_data=None, project_id=self.current_project, folder_id=folder_data["id"]
        )
        ...
        try:
            with self.folder_service.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    while current_id:
                        cursor.execute(
                            "SELECT parent_id FROM case_folders WHERE id = %s",
                            (current_id,),
                        )
    ...
    def add_case_folder(self):
        ...
        if self.current_folder:
            ...
            if parent_folder_id:
                ...
                if current_level >= 3:
                    Toast.warn(
                        self, "文件夹层级最多为3级，无法在当前文件夹下创建子文件夹"
                    )
                    return
        ...
        if data["type"] == "folder":
            ...
            if current_level < 2:
                ...
                add_folder_action.triggered.connect(
                    lambda: self.add_case_folder_with_parent(folder_id)
                )
                menu.addAction(add_folder_action)

            add_case_action = QAction("新建用例", self)
            add_case_action.triggered.connect(
                lambda: self.create_test_case_in_folder(data["data"])
            )
            ...
            delete_action.triggered.connect(
                lambda: self.delete_case_folder(data["data"])
            )
    ...
    def add_api_folder(self):
        ...
        dialog = ApiFolderDialog(
            self, project_id=self.current_project, parent_folder_id=None
        )
        if dialog.exec_() == QDialog.Accepted:
            data = dialog.get_data()
            if not data["name"]:
    ...
    def run_test_case(self, case_data):
        """执行测试用例"""
        if (
            not self.current_case_data
            or self.current_case_data["id"] != case_data["id"]
        ):
            # 如果当前没有加载这个用例，先加载
    ...
    def on_paste_shortcut(self):
        ...
        if selected_items:
            ...
            if data and data["type"] == "folder":
                ...
                if (
                    hasattr(self, "copied_case_folder_id")
                    and self.copied_case_folder_id is not None
                ):
    ...
    def paste_to_folder(self, folder_id):
        ...
        try:
            # 生成唯一的副本名称
            copy_name = self.generate_copy_name(
                self.copied_case_data["name"], folder_id
            )
            ...
            copy_data = {
                ...
                "enable_encryption": self.copied_case_data.get(
                    "enable_encryption", False
                ),
                "encrypt_url": self.copied_case_data.get("encrypt_url", ""),
                "decrypt_url": self.copied_case_data.get("decrypt_url", ""),
                "steps": self.copied_case_data.get(
                    "steps", []
                ),  # 修复：使用正确的字段名 'steps' 而不是 'test_steps'
            }
    ...
    def paste_to_root(self):
        ...
        try:
            ...
            # 生成唯一的副本名称
            copy_name = self.generate_copy_name(
                self.copied_case_data["name"], original_folder_id
            )
