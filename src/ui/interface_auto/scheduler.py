...
class SchedulerDialog(QDialog):
    ...
    def setup_basic_tab(self, parent):
        ...
        self.selected_case_list.setSizePolicy(
    ...
    def setup_notify_tab(self, parent):
        ...
        # 说明文本
        info_label = QLabel(
            "注意：邮件发送将使用全局邮箱配置，请先在系统设置中配置SMTP服务器。"
        )
    ...
    def load_folders(self):
        ...
        try:
            ...
            root_folders = self.folder_service.get_folders_by_project(
    ...
    def _build_folder_tree(
        self, folders: List[Dict[str, Any]], parent_item: Optional[QTreeWidgetItem]
    ):
        """递归构建文件夹树形结构"""
    ...
    def load_test_cases_by_folder(self, folder_id):
        ...
        try:
            ...
            # 从数据库动态获取指定文件夹的测试用例
            cases = self.test_case_service.get_cases_by_folder(
                self.current_project_id, folder_id
            )
    ...
    def remove_selected_cases(self):
        ...
        rows = sorted(
            [self.selected_case_list.row(item) for item in selected_items], reverse=True
        )
        for row in rows:
    ...
    def load_scheduler_data(self):
        ...
        if isinstance(notify_wechat, list):
            ...
            if notify_wechat:
                notify_wechat = (
                    notify_wechat[0] if isinstance(notify_wechat[0], dict) else {}
                )
            else:
                notify_wechat = {}
    ...
    def validate_and_accept(self):
        ...
        if self.is_edit:
            ...
            if scheduler_service.check_scheduler_name_exists(
                name, self.scheduler_data.get("id")
            ):
                Toast.warning(
                    self, "校验错误", f"调度名称 '{name}' 已存在，请使用其他名称"
                )
                self.name_edit.setFocus()
                return
        else:
    ...
    def get_data(self):
        ...
        if email_enabled:
            data["email_config"] = {"enabled": email_enabled}  # 记录邮件通知启用状态
        else:
            data["email_config"] = {}
    ...
    def update_cron_expression(self):
        ...
        try:
            ...
            if not is_valid:
                self.cron_edit.setStyleSheet(
                    "QLineEdit { border: 2px solid red; background-color: #fff0f0; }"
                )
                # 可以添加错误提示，但避免频繁弹窗
                # QMessageBox.warning(self, "Cron表达式错误", error_msg)
            else:
                self.cron_edit.setStyleSheet(
                    "QLineEdit { border: 2px solid green; background-color: #f0fff0; }"
                )
    ...
    def _update_second_tab(self, second_field):
        ...
        try:
            ...
            self.second_specific.currentTextChanged.disconnect(
                self.update_cron_expression
            )
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                self.second_from.valueChanged.connect(self.update_cron_expression)
                self.second_interval.valueChanged.connect(self.update_cron_expression)
                self.second_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def _update_minute_tab(self, minute_field):
        ...
        try:
            ...
            self.minute_specific.currentTextChanged.disconnect(
                self.update_cron_expression
            )
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                self.minute_from.valueChanged.connect(self.update_cron_expression)
                self.minute_interval.valueChanged.connect(self.update_cron_expression)
                self.minute_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def _update_hour_tab(self, hour_field):
        ...
        try:
            ...
            self.hour_specific.currentTextChanged.disconnect(
                self.update_cron_expression
            )
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                self.hour_from.valueChanged.connect(self.update_cron_expression)
                self.hour_interval.valueChanged.connect(self.update_cron_expression)
                self.hour_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def _update_day_tab(self, day_field):
        ...
        try:
            ...
            try:
                ...
                self.day_from.valueChanged.connect(self.update_cron_expression)
                self.day_interval.valueChanged.connect(self.update_cron_expression)
                self.day_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def _update_month_tab(self, month_field):
        ...
        try:
            ...
            self.month_specific.currentTextChanged.disconnect(
                self.update_cron_expression
            )
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                self.month_from.valueChanged.connect(self.update_cron_expression)
                self.month_interval.valueChanged.connect(self.update_cron_expression)
                self.month_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def _update_week_tab(self, week_field):
        ...
        try:
            ...
            self.week_specific.currentTextChanged.disconnect(
                self.update_cron_expression
            )
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                self.week_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def _update_year_tab(self, year_field):
        ...
        try:
            ...
            self.year_specific.currentTextChanged.disconnect(
                self.update_cron_expression
            )
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                self.year_specific.currentTextChanged.connect(
                    self.update_cron_expression
                )
            except:
                pass
    ...
    def validate_cron_field(self, field, field_index):
        ...
        if "-" in field:
            ...
            if not (
                self.check_field_range(start, field_index)
                and self.check_field_range(end, field_index)
            ):
                return False
            if start > end:
                return False
        ...
        if "/" in field:
            if field.startswith("*/"):
                ...
                if not (
                    self.check_field_range(base, field_index)
                    and self.check_field_range(step, field_index)
                ):
                    return False
                return step > 0
    ...
    def get_minute_cron(self):
        ...
        if self.minute_option1.isChecked():
            ...
            print(
                f"[DEBUG] 分钟字段：选项3被选中，开始值={from_val}, 间隔值={interval_val}"
            )
    ...
    def get_day_cron(self):
        ...
        # 避免递归调用，直接检查周字段的选项状态
        if (
            self.week_option3.isChecked()
            or self.week_option4.isChecked()
            or self.week_option5.isChecked()
            or self.week_option6.isChecked()
        ):
    ...
    def get_week_cron(self):
        ...
        # 避免递归调用，直接检查日字段的选项状态
        if (
            self.day_option3.isChecked()
            or self.day_option4.isChecked()
            or self.day_option5.isChecked()
            or self.day_option6.isChecked()
        ):
...
class SchedulerManager(QWidget):
    ...
    report_tab_requested = pyqtSignal(
        dict
    )  # 测试报告tab跳转请求信号，传递调度和报告数据
    ...
    def init_ui(self):
        ...
        project_label.setStyleSheet(
            "color: #333; font-weight: bold; margin-left: 5px; margin-right: 5px;"
        )
        ...
        self.refresh_action.triggered.connect(
            lambda: self.load_schedulers(self.project_combo.currentData())
        )
    ...
    def on_business_changed(self, business_id):
        ...
        # 重新加载调度列表
        project_id = (
            self.project_combo.currentData() if self.project_combo.count() > 0 else None
        )
        self.load_schedulers(project_id)
    ...
    def load_schedulers(self, project_id=None):
        ...
        try:
            # 使用分页查询获取调度数据
            schedulers, total_records = (
                self.scheduler_service.get_schedulers_with_pagination(
            ...
            self.total_pages = (
                total_records + self.current_page_size - 1
            ...
            for i, scheduler in enumerate(schedulers, start=1):
                ...
                last_run_text = (
                    last_run.strftime("%Y-%m-%d %H:%M:%S") if last_run else "从未执行"
                )
                ...
                next_run_text = (
                    next_run.strftime("%Y-%m-%d %H:%M:%S") if next_run else "未计算"
                )
                ...
                run_btn.setStyleSheet(
                    "QPushButton { border: none; background: transparent; padding: 0px; } QPushButton:hover { background: #e0e0e0; }"
                )
                run_btn.clicked.connect(
                    lambda checked, sched_id=scheduler["id"]: self.run_scheduler_by_id(
                        sched_id
                    )
                )
                ...
                edit_btn.clicked.connect(
                    lambda checked, sched_id=scheduler["id"]: self.edit_scheduler_by_id(
                        sched_id
                    )
                )
                ...
                toggle_btn.clicked.connect(
                    lambda checked, sched_id=scheduler[
                        "id"
                    ]: self.toggle_scheduler_by_id(sched_id)
                )
                ...
                copy_btn.clicked.connect(
                    lambda checked, sched_id=scheduler["id"]: self.copy_scheduler_by_id(
                        sched_id
                    )
                )
                ...
                delete_btn.clicked.connect(
                    lambda checked, sched_id=scheduler[
                        "id"
                    ]: self.delete_scheduler_by_id(sched_id)
                )
                ...
                detail_btn.clicked.connect(
                    lambda checked, sched_id=scheduler["id"]: self.show_execution_logs(
                        sched_id
                    )
                )
    ...
    def on_scheduler_executed(self, scheduler_data, success, message):
        ...
        # 显示Toast提示
        status_text = (
            f"调度 '{scheduler_name}' 执行{'成功' if success else '失败'}: {message}"
        )
        if success:
    ...
    def add_scheduler(self):
        ...
        if current_project_id is not None:
            ...
            QTimer.singleShot(
                100,
                lambda: self._preselect_project_in_dialog(dialog, current_project_id),
            )
    ...
    def delete_scheduler(self, scheduler_data):
        ...
        if scheduler_data.get("enabled", False):
            Toast.warn(
                self,
                f"调度 '{scheduler_data['name']}' 当前处于启用状态，请先禁用后再删除",
            )
            return
    ...
    def run_scheduler(self, scheduler_data):
        ...
        try:
            ...
            Toast.info(
                self,
                f"开始执行调度: {scheduler_data['name']} (包含 {len(case_ids)} 个测试用例)",
            )
        ...
        try:
            ...
            for i, case_id in enumerate(case_ids):
                try:
                    ...
                    # 连接信号
                    execution_thread.case_finished.connect(
                        self._on_case_execution_finished
                    )
            ...
            if success_count == total_count:
                Toast.success(
                    self, f"调度执行完成: 成功 {success_count}/{total_count} 个测试用例"
                )
            else:
                Toast.warn(
                    self, f"调度执行完成: 成功 {success_count}/{total_count} 个测试用例"
            ...
            if notify_emails:
                try:
                    ...
                    Toast.info(
                        self, f"测试报告邮件已发送至: {', '.join(notify_emails)}"
                    )
    ...
    def toggle_scheduler(self, scheduler_data):
        ...
        try:
            self.scheduler_service.update_scheduler_status(
                scheduler_data["id"], new_status
            )
            self.load_schedulers(self.project_combo.currentData())
            self.data_changed.emit()
    ...
    def copy_scheduler(self, scheduler_data):
        ...
        try:
            ...
            # 验证名称是否重复
            existing_names = [
                s["name"] for s in self.scheduler_service.get_all_schedulers()
            ]
    ...
    def _jump_to_test_report_fallback(self, scheduler_data, reports):
        ...
        try:
            ...
            if parent and hasattr(parent, "switch_to_test_report_tab"):
                ...
                Toast.info(
                    self,
                    f"已跳转到测试报告tab，调度 '{scheduler_data['name']}' 共有 {len(reports)} 条执行记录",
                )
    ...
    def _view_report_detail(self, table_widget, parent_dialog):
        ...
        try:
            ...
            scheduler_id = (
                self.current_scheduler_id
                if hasattr(self, "current_scheduler_id")
                else None
            )
            reports = (
                report_service.get_reports_by_scheduler_id(scheduler_id)
                if scheduler_id
                else []
            )
    ...
    def _open_report_detail_fallback(self, report_data):
        ...
        try:
            ...
            detail_dialog.setWindowTitle(
                f"测试报告 - {report_data.get('report_name', '')}"
            )
