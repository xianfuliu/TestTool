    ...
    def stop(self):
        ...
        try:
            self.log_message.emit(
                self.format_debug_message("线程已安全停止", "debug", -1), "debug", -1
            )
        except:
            pass  # 忽略发送消息时的错误
    ...
    def run(self):
        ...
        try:
            ...
            def log_callback(message, level, step_index):
                # 使用原有的日志格式化方法
                formatted_message = self.format_debug_message(
                    message, level, step_index
                )
            ...
            # 记录用例执行异常信息
            self.log_message.emit(
                self.format_debug_message(f"用例执行异常: {str(e)}", "error", -1),
                "error",
                -1,
            )
            self.case_finished.emit(
    ...
    def extract_field_value(self, step_result, field_name):
        ...
        if decrypted_body:
            ...
            try:
                ...
                # 如果解析失败，使用原始响应数据
                response_data = step_result.get(
                    "response_data", step_result.get("body", {})
                )
                response_text = step_result.get(
        ...
        response_headers = step_result.get(
            "response_headers", step_result.get("headers", {})
        )
...
class CaseTabWidget(QWidget):
    ...
    def setup_steps_tab(self, parent):
        ...
        self.steps_scroll.setSizePolicy(
        ...
        self.steps_placeholder.setStyleSheet(
        ...
        self.steps_widget.customContextMenuRequested.connect(
    ...
    def sync_all_step_cards_encryption_status(self, enable_encryption):
        ...
        for i in range(self.steps_layout.count()):
            ...
            if item and item.widget():
                ...
                elif hasattr(widget, "encryption_btn") and hasattr(
                    widget.encryption_btn, "setChecked"
                ):
                    # 直接设置加解密按钮状态
            ...
            if item and item.widget():
                ...
                if hasattr(widget, "step_id") and hasattr(widget, "step_data"):
                    ...
                    if i < len(case_data.get("steps", [])):
                        ...
                        if "enable_encryption" in step_data:
                            temp_step["enable_encryption"] = step_data[
                                "enable_encryption"
                            ]
    ...
    def save_case(self):
        ...
        # 保存加解密配置
        self.current_case.enable_encryption = (
            self.enable_encryption_checkbox.isChecked()
        )
        ...
        if (
            self.current_case
            and self.current_case.steps
            and hasattr(self, "steps_layout")
        ):
            # 获取布局中所有步骤的前端ID顺序
            current_step_ids = []
            for i in range(self.steps_layout.count()):
        ...
        if self.is_edit:
            ...
            # 如果self.case_data中没有ID，尝试从current_case获取
                ...
                hasattr(self, "current_case")
                and self.current_case
                and hasattr(self.current_case, "id")
                and self.current_case.id
            ):
                exclude_id = self.current_case.id

        name_exists = case_service.check_case_name_exists(
            self.project_id, self.current_case.name, self.folder_id, exclude_id
        )
        ...
        if name_exists:
            Toast.warning(
                self,
                "警告",
                f"测试用例名称 '{self.current_case.name}' 已存在，请使用其他名称",
            )
            return
    ...
    def cancel(self):
        ...
        if self.modified:
            # 创建确认对话框，手动设置按钮文本
            msg_box = QMessageBox(
                QMessageBox.Question, "确认取消", "有未保存的修改，确定要取消吗？"
            )
        ...
        if mime_data.hasFormat("application/json"):
            try:
                ...
                if drag_data.get("type") == "api_template":
                    ...
                    if template_id:
                        # 从服务中获取完整的模板数据
                        from src.core.services.api_template_service import (
                            ApiTemplateService,
                        )
    ...
    def add_api_template_to_steps(self, template_data, drop_position=None):
        ...
        existing_orders = {
            step.step_order
            for step in (self.current_case.steps if self.current_case else [])
        }
        ...
        step_data_for_model = {
            ...
            # 根据全局加解密配置设置默认值
            "enable_encryption": (
                bool(self.case_data.get("enable_encryption", False))
                if self.case_data
                else False
            ),
        }
    ...
    def add_step_card(self, step_data):
        ...
        step_card.mousePressEvent = (
            lambda event, card=step_card: self.on_step_card_clicked(event, card)
        )
    ...
    def on_step_updated(self, step_data):
        ...
        if hasattr(self, "case_data") and self.case_data:
            ...
            for i, step in enumerate(steps):
                if step.get("id") == step_data.get("id") or step.get(
                    "name"
                ) == step_data.get("name"):
                    # 更新步骤数据，特别确保启用状态同步
                    steps[i] = step_data
    ...
    def on_step_moved(self, dragged_step_id, target_step_id, from_index, to_index):
        ...
        if from_index >= 0 and to_index >= 0 and from_index != to_index:
            # 检查索引是否在有效范围内
            if from_index >= len(self.current_case.steps) or to_index > len(
                self.current_case.steps
            ):
                return
    ...
    def on_step_copied(self, step_id, copied_step_data):
        ...
        try:
            ...
            insert_index = (
                ...
                else len(self.current_case.steps)
            )
    ...
    def log_debug_info(self):
        ...
        for i, step in enumerate(steps):
            ...
            step_name = (
                step_dict.get("api_name")
                or step_dict.get("api_template", {}).get("name")
                or step_dict.get("name", "未命名步骤")
            )
    ...
    def start_execution(self):
        ...
        # 记录开始执行日志
        self.log_message(
            f"开始执行测试用例: {case_data.get('name', '未知用例')}", "info"
        )
    ...
    def get_environment_config(self):
        ...
        if environment_id:
            try:
                environment = self.environment_service.get_environment_by_id(
                    environment_id
                )
                if environment:
                    return {
        ...
        # 返回默认配置
        return {"base_url": "", "headers": {}, "variables": {}}
    ...
    def log_message_with_step(self, message, level="info", step_index=-1):
        ...
        if (
            step_index >= 0
            and self.current_case
            and len(self.current_case.steps) > step_index
        ):
        ...
        if hasattr(self, "execution_logs_dialog") and self.execution_logs_dialog:
            try:
                ...
                if (
                    ...
                    if hasattr(step, "api_name"):
                        step_name = step.api_name
                    elif hasattr(step, "api_template") and hasattr(
                        step.api_template, "name"
                    ):
                        step_name = step.api_template.name
                    elif hasattr(step, "name"):
                        step_name = step.name
                ...
                self.execution_logs_dialog.add_log_with_step(
                    message, level, step_index, step_name
                )
            except RuntimeError:
                # 弹窗已被删除，忽略错误
                pass
...
class TabbedCaseEditor(QWidget):
    ...
    def open_case(self, case_data=None, project_id=None, folder_id=None):
        ...
        if case_data and "id" in case_data and case_data["id"]:
            ...
            for existing_tab_id, tab_data in self.tabs.items():
                ...
                if (
                    hasattr(widget, "case_data")
                    and widget.case_data
                    and "id" in widget.case_data
                    and widget.case_data["id"] == case_id
                ):
        ...
        editor_widget.api_template_edit_requested.connect(
    ...
    def generate_tab_id(self, case_data):
        ...
        if case_data and "id" in case_data and case_data["id"]:
            ...
            # 使用计数器来确保新标签页ID的唯一性
            new_case_count = sum(
                1 for tab_id in self.tabs.keys() if tab_id.startswith("new_case_")
            )
    ...
    def show_execution_logs(self):
        ...
        if current_logs:
            for log_entry in current_logs:
                ...
                self.execution_logs_dialog.add_log_with_step(
                    log_entry["message"], log_entry["level"], step_index, step_name
                )
        else:
            # 添加一些示例日志
    ...
    def add_execution_log(self, message, level="info"):
        ...
        # 如果弹窗存在，向弹窗添加日志
        if (
            hasattr(self, "execution_logs_dialog")
            and self.execution_logs_dialog is not None
        ):
    ...
    def case_saved(self, tab_id, case_data):
        ...
        if tab_id not in self.tabs:
            ...
            for existing_tab_id, existing_tab_data in self.tabs.items():
                if (
                    hasattr(existing_tab_data["widget"], "case_data")
                    and existing_tab_data["widget"].case_data == case_data
                ):
...
class StepLogItem(QWidget):
    ...
    def init_ui(self):
        ...
        self.log_count_label.setStyleSheet(
            "color: #666; font-size: 12px; margin-left: 4px;"
        )
        ...
        self.status_icon_label.setStyleSheet(
            "margin-left: 0px; margin-right: 8px; padding: 0px; background: transparent;"
        )
    ...
    def add_log(self, message, level="info"):
        ...
        # 保存到日志列表
        self.logs.append(
            {"timestamp": timestamp, "level": actual_level, "message": message}
        )
        ...
        if self.step_status is None:
            ...
            # 如果有成功或断言通过的信息，标记为成功
                ...
                or "断言通过" in message
                or "assertion passed" in message.lower()
                or "执行完成: success" in message
            ):
                self.step_status = True
                self.update_header_style()
    ...
    def update_status_icon(self):
        ...
        if self.step_status is None:
            ...
            self.status_icon_label.setPixmap(
                success_icon.pixmap(16, 16)
            )  # 增加图标大小以适应200x200源文件
        else:
            # 执行报错：显示失败图标
            fail_icon = self.get_icon("fail.png")
            self.status_icon_label.setPixmap(
                fail_icon.pixmap(16, 16)
            )  # 增加图标大小以适应200x200源文件
