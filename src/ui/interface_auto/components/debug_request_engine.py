        ...
        try:
            ...
            if method == "GET":
                ...
                response = session.post(
                    url=url, params=params, json=body, headers=headers, timeout=timeout
                )
            elif method == "PUT":
            ...
            self.error.emit(
                f"请求超时 (timeout={self.request_data.get('timeout', 30)}秒)"
            )
...
class DebugRequestEngine:
    ...
    def execute_debug_request(
        self, request_data: Dict[str, Any], finished_callback=None, error_callback=None
    ):
        """执行调试请求"""
        # 停止之前的worker
