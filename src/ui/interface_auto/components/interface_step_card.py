...
class InterfaceStepCard(QFrame):
    ...
    step_moved = pyqtSignal(
        str, str, int, int
    )  # dragged_step_id, target_step_id, from_index, to_index
    ...
    def get_icon_pixmap(self, icon_name, width=16, height=16):
        ...
        try:
            ...
            if os.path.exists(dev_path):
                return QPixmap(dev_path).scaled(
            ...
            if os.path.exists(exe_path):
                return QPixmap(exe_path).scaled(
                    width, height, Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
            ...
            if os.path.exists(relative_path):
                return QPixmap(relative_path).scaled(
                    width, height, Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
            ...
            if getattr(sys, "frozen", False):
                ...
                if os.path.exists(meipass_path):
                    return QPixmap(meipass_path).scaled(
                        width, height, Qt.KeepAspectRatio, Qt.SmoothTransformation
                    )
    ...
    def create_interface_display(self):
        ...
        # 请求方式
        method = self.step_data.get("api_method") or self.step_data.get(
            "api_template", {}
        ).get("method", "GET")
        ...
        api_name = self.step_data.get("api_name") or self.step_data.get(
            "api_template", {}
        ).get("name", "未命名接口")
        ...
        self.api_name_label.mousePressEvent = (
            lambda event: self.on_api_name_clicked()
        )  # 连接点击事件
        ...
        try:
            ...
            while parent:
                ...
                if hasattr(parent, "case_data"):
                    ...
                    global_enable_encryption = parent.case_data.get(
    ...
    def on_api_name_clicked(self):
        ...
        api_template_id = self.step_data.get("api_template", {}).get(
            "id"
        ) or self.step_data.get("api_template_id")
        if api_template_id:
    ...
    def show_pre_tool_menu(self):
        ...
        for tool in tools:
            ...
            action.triggered.connect(
                lambda checked, tool_name=tool: self.on_pre_tool_selected(tool_name)
            )
    ...
    def show_post_tool_menu(self):
        ...
        for tool in post_tools:
            action = menu.addAction(tool)
            action.triggered.connect(
                lambda checked, tool_name=tool: self.on_post_tool_selected(tool_name)
            )
    ...
    def add_http_request_tool(self):
        ...
        try:
            # 生成唯一的工具ID
            tool_id = (
                f"http_request_{len(self.step_data.get('pre_processing', {})) + 1}"
            )
            ...
            default_config = {
                ...
                "enabled": True,
                "priority": len(
                    self.step_data.get("pre_processing", {})
                ),  # 添加优先级字段，按添加顺序排序
            }

            # 直接打开编辑对话框
            from src.ui.interface_auto.dialogs.http_request_dialog import (
                HttpRequestDialog,
            )
            ...
            # 连接保存信号
            dialog.request_saved.connect(
                lambda config_data: self.on_http_request_added(tool_id, config_data)
            )
    ...
    def add_sql_tool(self):
        ...
        try:
            ...
            # 连接保存信号
            dialog.sql_tool_saved.connect(
                lambda config_data: self.on_sql_tool_added(tool_id, config_data)
            )
    ...
    def add_assertion_tool(self, assertion_type, assertion_name):
        ...
        try:
            ...
            # 连接保存信号
            dialog.assertion_saved.connect(
                lambda config_data: self.on_assertion_added(tool_id, config_data)
            )
    ...
    def add_post_tool(self, tool_type, tool_name):
        ...
        try:
            ...
            default_config = {
                ...
                "enabled": True,
                "priority": len(
                    self.step_data.get("post_processing", {})
                ),  # 添加优先级字段，按添加顺序排序
            }
            ...
            # 直接打开编辑对话框
            from src.ui.interface_auto.dialogs.parameter_extraction_dialog import (
                ParameterExtractionDialog,
            )

            dialog = ParameterExtractionDialog(self, default_config)

            # 连接保存信号
            dialog.extraction_saved.connect(
                lambda config_data: self.on_post_tool_added(tool_id, config_data)
            )
    ...
    def refresh_pre_tools_display_with_layout(self, layout):
        ...
        if not pre_processing:
            ...
            no_tools_label.setStyleSheet(
                "color: #999; font-style: italic; padding: 10px; font-size: 12px;"
            )
    ...
    def refresh_assertion_tools_display_with_layout(self, layout):
        ...
        # 按照优先级字段对工具进行排序
        sorted_tools = sorted(assertions.items(), key=lambda x: x[1].get("priority", 0))
    ...
    def add_http_request_tool_widget(self, tool_id, tool_config, parent_layout):
        ...
        edit_btn.clicked.connect(
            lambda checked, tid=tool_id: self.edit_http_request_tool(tid)
        )
        ...
        delete_btn.clicked.connect(
            lambda checked, tid=tool_id: self.delete_pre_tool(tid)
        )
    ...
    def add_assertion_tool_widget(self, tool_id, tool_config, parent_layout):
        ...
        copy_btn.clicked.connect(
            lambda checked, tid=tool_id: self.copy_assertion_tool(tid)
        )
        ...
        edit_btn.setToolTip("编辑")
        edit_btn.clicked.connect(
            lambda checked, tid=tool_id: self.edit_assertion_tool(tid)
        )
        ...
        delete_btn.setToolTip("删除")
        delete_btn.clicked.connect(
            lambda checked, tid=tool_id: self.delete_assertion_tool(tid)
        )
    ...
    def add_sql_tool_widget(self, tool_id, tool_config, parent_layout):
        ...
        delete_btn.setToolTip("删除")
        delete_btn.clicked.connect(
            lambda checked, tid=tool_id: self.delete_sql_tool(tid)
        )
    ...
    def add_post_tool_widget(self, tool_id, tool_config, parent_layout):
        ...
        delete_btn.setToolTip("删除")
        delete_btn.clicked.connect(
            lambda checked, tid=tool_id: self.delete_post_tool(tid)
        )
    ...
    def edit_http_request_tool(self, tool_id):
        ...
        try:
            ...
            # 连接保存信号
            dialog.request_saved.connect(
                lambda config_data: self.on_http_request_edited(tool_id, config_data)
            )
    ...
    def edit_assertion_tool(self, tool_id):
        ...
        try:
            ...
            # 连接保存信号
            dialog.assertion_saved.connect(
                lambda config_data: self.on_assertion_edited(tool_id, config_data)
            )
    ...
    def edit_post_tool(self, tool_id):
        ...
        try:
            ...
            # 连接保存信号
            dialog.extraction_saved.connect(
                lambda config_data: self.on_post_processing_edited(tool_id, config_data)
            )
    ...
    def edit_sql_tool(self, tool_id):
        ...
        try:
            ...
            # 连接保存信号
            dialog.sql_tool_saved.connect(
                lambda config_data: self.on_sql_tool_edited(tool_id, config_data)
            )
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() == QEvent.MouseButtonPress:
            ...
            # 阻止Tab容器内部的鼠标按下事件传递到步骤卡片
                ...
                obj == self.pre_tab or obj == self.assertion_tab or obj == self.post_tab
            ):
                # Tab容器内部按下，阻止事件传递
                return True
            ...
            if (
                ...
                and hasattr(self, "dragged_widget")
                and self.dragged_widget
                and hasattr(self, "dragged_tool_id")
                and self.dragged_tool_id
                and hasattr(self, "drag_start_position")
                and self.drag_start_position is not None
                and (event.pos() - self.drag_start_position).manhattanLength()
            ...
            # 处理Tab上的鼠标移动（用于更新指示器）
                ...
                (
                    obj == self.pre_tab
                    or obj == self.assertion_tab
                    or obj == self.post_tab
                )
                and hasattr(self, "dragging_widget")
                and self.dragging_widget
            ):
                self.update_drag_indicator(event.pos())
                return True
        ...
        if insert_index < layout.count():
            ...
            if target_widget:
                ...
                try:
                    self.drag_indicator.setGeometry(
                        target_rect.x(), target_rect.y() - 2, target_rect.width(), 4
                    )
                except RuntimeError:
                    # 如果拖动指示器已被删除，直接返回
                    return
            ...
            if layout.count() > 0:
                ...
                if last_widget:
                    ...
                    try:
                        self.drag_indicator.setGeometry(
                            last_rect.x(), last_rect.bottom() + 2, last_rect.width(), 4
                        )
                    except RuntimeError:
                        # 如果拖动指示器已被删除，直接返回
                        return
    ...
    def end_drag(self):
        ...
        if (
            hasattr(self, "dragging_widget")
            and self.dragging_widget
            and hasattr(self, "dragging_tool_id")
            and self.dragging_tool_id
        ):
            # 检查widget是否仍然有效
                        ...
                        if dragged_index >= 0 and target_index >= 0:
                            ...
                            if dragged_index == target_index:
                                ...
                                if dragged_index < target_index:
                                    ...
                                    if (
                                        hasattr(self, "step_drag_insert_before")
                                        and not self.step_drag_insert_before
                                    ):
                                        target_index = target_index - 1

                                # 发送新的移动信号，包含前端ID信息
                                self.step_moved.emit(
                                    dragged_step_id,
                                    self.step_id,
                                    dragged_index,
                                    target_index,
    ...
    def _remove_drag_visual_feedback(self):
        ...
        for i in range(parent_layout.count()):
            ...
            if item and item.widget():
                widget = item.widget()
                if hasattr(widget, "_original_style") and hasattr(
                    widget, "setStyleSheet"
                ):
