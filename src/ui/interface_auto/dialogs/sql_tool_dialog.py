...
class SQLToolDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.conn_status_label.setStyleSheet("color: gray; font-size: 12px; margin-left: 10px;")
        ...
        self.sql_edit.setPlaceholderText("请输入SELECT查询语句，支持${变量名}格式引用变量")
    ...
    def auto_connect_database(self):
        ...
        try:
            ...
            self.test_worker = SQLWorker("auto_connect", self.connection_params, test_sql)
    ...
    def on_auto_connect_error(self, query_name, error_message):
        """自动连接失败"""
        error_msg = error_message.split(":")[0] if ":" in error_message else error_message
    ...
    def load_database_list(self):
        ...
        try:
            ...
            self.db_list_worker = SQLWorker("get_databases", self.connection_params, db_list_sql)
    ...
    def set_default_databases(self):
        """设置默认数据库列表"""
        self.db_combo.clear()
        default_databases = ["test_db", "mysql", "information_schema", "performance_schema"]
    ...
    def load_existing_config(self):
        ...
        if self.sql_config:
            ...
            if output_fields:
                output_str = ",".join([field.get("field", "") for field in output_fields])
    ...
    def on_validate_success(self, query_name, message, result_data):
        ...
        Toast.information(self, "成功", f"SQL语句校验通过，返回 {len(result_data)} 行数据")
