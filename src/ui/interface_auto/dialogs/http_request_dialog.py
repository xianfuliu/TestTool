...
class HttpRequestDialog(QDialog):
    ...
    def create_content_widget(self):
        ...
        self.variable_scroll_area.setStyleSheet(
    ...
    def add_variable_row(self, insert_after_row=None):
        ...
        add_button.clicked.connect(
            lambda checked, row=row_data: self.add_variable_row(row)
        )
...
"""
    ...
    def _get_current_data(self):
        ...
        try:
            ...
            # 获取请求头和变量配置
            headers = (
                self.get_headers_from_rows()
                if hasattr(self, "get_headers_from_rows")
                else {}
            )
            variables = (
                self.get_variables_from_rows()
                if hasattr(self, "get_variables_from_rows")
                else {}
            )
    ...
    def closeEvent(self, event):
        ...
        if self._has_unsaved_changes():
            # 创建确认对话框，手动设置按钮文本
            msg_box = QMessageBox(
                QMessageBox.Question, "未保存的修改", "您有未保存的修改，是否保存？"
            )
