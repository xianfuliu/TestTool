...
class VariableDialog(QDialog):
    ...
    def is_variable_name_duplicate(self, name):
        ...
        # 获取父级变量管理对象的服务
        if (
            hasattr(self.parent_variable_management, "variable_service")
            and self.parent_variable_management.variable_service
        ):
            variable_service = self.parent_variable_management.variable_service
            try:
                # 检查变量名是否已存在（在同一项目内）
                existing_variable = variable_service.get_global_variable_by_name(
                    name, self.project_id
                )
                if existing_variable:
                    # 如果是编辑模式，且找到的变量就是正在编辑的变量，则不算重复
                    if self.is_edit and existing_variable.get(
                        "id"
                    ) == self.variable_data.get("id"):
                        return False
                    return True
...
class VariableManagement(QWidget):
    ...
    def init_ui(self):
        ...
        self.project_info_label.setStyleSheet(
            "color: #666; font-size: 14px; margin: 20px;"
        )
    ...
    def setup_global_tab(self, parent):
        ...
        self.global_tree.setHeaderLabels(
            ["序号", "变量名", "类型", "值", "描述", "操作"]
        )
    ...
    def load_global_variables(self):
        ...
        try:
            # 加载当前项目的全局变量
            global_vars = self.variable_service.get_global_variables_by_project(
            ...
            for row, var in enumerate(global_vars):
                ...
                edit_btn.clicked.connect(
                    lambda checked=False, r=row: self.edit_global_variable_by_row(r)
                )
                ...
                delete_btn.clicked.connect(
                    lambda checked=False, r=row: self.delete_global_variable_by_row(r)
                )
                ...
                copy_btn.clicked.connect(
                    lambda checked=False, r=row: self.copy_global_variable_by_row(r)
                )
    ...
    def edit_global_variable_by_row(self, row):
        ...
        try:
            # 获取变量数据
            variables = self.variable_service.get_global_variables_by_project(
                self.current_project["id"]
            )
            if row < len(variables):
                ...
                if dialog.exec_() == QDialog.Accepted:
                    ...
                    # 更新变量
                    self.variable_service.update_global_variable(
                        variable_data["id"], data
                    )
        ...
        try:
            ...
            if row < len(variables):
                ...
                if msg_box.clickedButton() == confirm_btn:
                    # 使用支持项目维度的删除方法
                    self.variable_service.delete_global_variable_by_name(
                        variable_data["name"], self.current_project["id"]
                    )
    ...
    def copy_global_variable_by_row(self, row):
        ...
        try:
            ...
            if row < len(variables):
                ...
                while True:
                    try:
                        existing = self.variable_service.get_global_variable_by_name(
                            new_variable_data["name"], self.current_project["id"]
                        )
                        if existing:
                ...
                Toast.success(
                    self, f"变量复制成功，新变量名为: {new_variable_data['name']}"
                )
                self.load_global_variables()
                self.data_changed.emit()
        except Exception as e:
