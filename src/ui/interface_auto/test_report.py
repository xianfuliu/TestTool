...
class PieChartWidget(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        if distance <= size // 2:
            ...
            if self.passed_cases > 0 and angle < current_angle + passed_angle:
                ...
                self.failed_cases > 0
                and angle < current_angle + passed_angle + failed_angle
            ):
                self.hovered_slice = "failed"
            elif (
                self.error_cases > 0
                and angle < current_angle + passed_angle + failed_angle + error_angle
            ):
    ...
    def paintEvent(self, event):
        ...
        if self.passed_cases > 0:
            color = (
                self.hover_colors["passed"]
                if self.hovered_slice == "passed"
                else self.colors["passed"]
            )
        ...
        if self.failed_cases > 0:
            color = (
                self.hover_colors["failed"]
                if self.hovered_slice == "failed"
                else self.colors["failed"]
            )
            painter.setPen(QPen(QColor(255, 255, 255, 180), 3))
            painter.setBrush(QBrush(color))
        ...
        if self.error_cases > 0:
            color = (
                self.hover_colors["error"]
                if self.hovered_slice == "error"
    ...
    def draw_tooltip(self, painter, event):
        ...
        painter.drawRoundedRect(
            tooltip_x, tooltip_y, tooltip_width, tooltip_height, 5, 5
        )
...
class ReportDetailPage(QWidget):
    ...
    def setup_summary_section(self, parent_layout):
        ...
        basic_layout.setContentsMargins(
            5, 0, 15, 0
        )  # 调整内边距，左内边距减小以与报告详情对齐
        ...
        self.report_start_time_label.setStyleSheet(
            "font-size: 18px; font-weight: normal;"
        )
        self.report_end_time_label = QLabel()
        self.report_end_time_label.setStyleSheet(
            "font-size: 18px; font-weight: normal;"
        )
        self.report_duration_label = QLabel()
        self.report_duration_label.setStyleSheet(
            "font-size: 18px; font-weight: normal;"
        )
    ...
    def _get_case_status(self, steps):
        ...
        status_priority = {"error": 4, "failure": 3, "skipped": 2, "success": 1}
    ...
    def show_step_logs_dialog(self, step_data):
        ...
        try:
            # 优先使用step_name字段，如果为空则使用api_name字段，最后使用默认值
            step_name = step_data.get("step_name") or step_data.get(
                "api_name", "未知接口"
            )
    ...
    def _create_case_items(self, case_groups):
        ...
        for case_id, steps in case_groups.items():
            ...
            case_name = (
                steps[0].get("case_name", f"用例 {case_id}")
                if steps
                else f"用例 {case_id}"
            )
            ...
            if case_name.startswith("用例 "):
                # 尝试从步骤数据中获取实际用例名称
                actual_case_name = (
                    steps[0].get("case_name", case_name) if steps else case_name
                )
...
class CaseItem(QWidget):
    ...
    def init_ui(self):
        ...
        self.case_label.setStyleSheet(
            "font-weight: 600; font-size: 16px; color: #24292f;"
        )
    ...
    def toggle_expand(self):
        ...
        for step_index, logs_data in self.step_logs_containers.items():
            ...
            print(
                f"[DEBUG] 步骤 {step_index}: step_expanded={step_expanded}, should_show={should_show}"
            )
    ...
    def load_case_steps(self):
        ...
        for step_index, step_data in enumerate(steps):
            ...
            api_name = (
                ...
                or step_data.get("name", "未知接口")
            )
            step_display_name = api_name  # 直接使用接口名称

            print(
                f"[DEBUG] 步骤 {step_index}: api_name={step_data.get('step_name')}, api_template.name={step_data.get('api_template', {}).get('name')}, name={step_data.get('name')}, 最终显示名称: {step_display_name}"
            )
    ...
    def on_step_expand_changed(self, step_item, is_expanded):
        ...
        print(
            f"[DEBUG] on_step_expand_changed: step_index={step_index}, is_expanded={is_expanded}, self.is_expanded={self.is_expanded}"
        )
...
class StepItem(QWidget):
    ...
    def __init__(
        self, step_name, step_index, step_data, parent=None, case_item_ref=None
    ):
        super().__init__(parent)
    ...
    def init_ui(self):
        ...
        self.step_label.setStyleSheet(
            "font-weight: 500; font-size: 15px; color: #24292f;"
        )
        ...
        # 计算文本格式日志的行数（非空行）
        log_lines = [
            line.strip() for line in execution_logs.strip().split("\n") if line.strip()
        ]
    ...
    def toggle_expand(self):
        ...
        print(
            f"[DEBUG] StepItem toggle_expand: step_index={self.step_index}, is_expanded={self.is_expanded}"
        )
        ...
        # 如果case_item_ref不存在，尝试使用父组件
            ...
            try:
                ...
                # 检查父组件是否有其他相关方法
                print(
                    f"[DEBUG] 父组件方法列表: {[method for method in dir(parent) if not method.startswith('_')]}"
                )
...
class StepLogItem(QWidget):
        ...
        if isinstance(execution_logs, str):
            ...
            for log_line in log_lines:
                if log_line.strip():  # 跳过空行
                    ...
                    # 使用整行颜色显示日志
                    log_entry = (
                        f"<span style='color: {color};'>{processed_message}</span>"
                    )
                    ...
                    # 保存到日志列表
                    self.logs.append(
                        {"timestamp": "", "level": "info", "message": log_line}
                    )
        else:
            # 兼容旧格式（字典列表）
            for log in execution_logs:
    ...
    def update_header_style(self):
        ...
        if self.step_status is None:
            ...
            self.step_label.setStyleSheet(
                "font-weight: 600; font-size: 13px; color: #ff0000;"
            )
...
class TestReportManager(QWidget):
    ...
    def load_schedulers(self):
        ...
        try:
            ...
            # 根据项目筛选调度任务
            schedulers = self.scheduler_service.get_schedulers_for_test_report(
                project_id
            )
    ...
    def on_reports_loaded(self, reports, request_id=None):
        ...
        try:
            ...
            if request_id is not None and request_id != self.current_request_id:
                print(
                    f"忽略过期请求: {request_id}，当前请求ID: {self.current_request_id}"
                )
                return
            ...
            for i, report in enumerate(reports, start=1):
                ...
                view_btn.clicked.connect(
                    lambda checked, report_id=report[
                        "id"
                ...
                delete_btn.clicked.connect(
                    lambda checked, report_id=report["id"]: self.delete_report_by_id(
                        report_id
                    )
                )
        ...
        try:
            ...
            if reports:
                ...
                for i, report in enumerate(reports):
                    print(
                        f"  {i+1}. ID: {report.get('id', 'N/A')}, 名称: {report.get('name', 'N/A')}, 状态: {report.get('status', 'N/A')}"
                    )
            else:
                print("没有找到符合条件的报告")
            ...
            if current_scheduler_id:
                for i in range(self.scheduler_list_widget.count()):
                    ...
                    if (
                        ...
                        and item.data(Qt.UserRole) == current_scheduler_id
                    ):
        ...
        try:
            ...
            if scheduler_id:
                ...
                for i in range(self.scheduler_list_widget.count()):
                    ...
                    if (
                        item.flags() != Qt.NoItemFlags
                        and item.data(Qt.UserRole) == scheduler_id
                    ):
                        self.scheduler_list_widget.setCurrentItem(item)
                        break
    ...
    def delete_scheduler_reports(self, scheduler_data):
        ...
        # 确认删除
        msg_box = QMessageBox(
            QMessageBox.Question, "确认删除", "确定要删除这个调度组的所有测试报告吗？"
        )
        ...
        if reply == QMessageBox.Yes:
            try:
                # 如果是调度组标识，需要先获取调度组名称
                if isinstance(scheduler_data, str) and scheduler_data.startswith(
                    "scheduler_"
                ):
    ...
    def delete_report_by_id(self, report_id):
        ...
        # 确认删除
        msg_box = QMessageBox(
            QMessageBox.Question, "确认删除", "确定要删除这个测试报告吗？"
        )
        confirm_button = msg_box.addButton("确认", QMessageBox.YesRole)
        cancel_button = msg_box.addButton("取消", QMessageBox.NoRole)
        msg_box.setDefaultButton(cancel_button)
    ...
    def clear_old_reports(self):
        ...
        try:
            ...
            days, ok = QInputDialog.getInt(
                self, "清理报告", "保留最近多少天的报告:", 30, 1, 365, 1
            )
    ...
    def toggle_scheduler_status(self, scheduler_id, enable):
        ...
        try:
            # 更新调度状态
            success = self.scheduler_service.update_scheduler_status(
                scheduler_id, enable
            )
