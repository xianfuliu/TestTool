    ...
    def test_email_connection(self, config: EmailConfig) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"编码错误详情 - 服务器: {safe_server}, 用户名: {safe_username}"
                )
            except:
            ...
            if "utf-8" in error_msg.lower() and "codec" in error_msg.lower():
                ...
                try:
                    safe_server = smtp_server.encode("utf-8", "ignore").decode("utf-8")
                    safe_username = smtp_username.encode("utf-8", "ignore").decode(
                        "utf-8"
                    )
                    logger.debug(
                        f"UTF-8编码错误详情 - 服务器: {safe_server}, 用户名: {safe_username}"
                    )
                except:
                ...
                try:
                    ...
                    logger.debug(
                        f"ASCII编码错误详情 - 服务器: {safe_server}, 用户名: {safe_username}"
                    )
                except:
                ...
                try:
                    ...
                    logger.debug(
                        f"其他错误详情 - 服务器: {safe_server}, 用户名: {safe_username}"
                    )
                except:
