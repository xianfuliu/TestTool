...
class UserService:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("SELECT id FROM users WHERE username = 'admin' AND is_admin = TRUE")
    ...
    def authenticate_user(self, username: str, password: str, create_session: bool = True) -> Optional[dict]:
        """用户认证"""
        try:
            ...
            session_token = None
            if create_session:
                session = self.create_session(user.id, expires_hours=24 * 7)  # 7天有效期
                if session:
