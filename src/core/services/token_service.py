...
class TokenService:
    ...
    def create_session(self, user_id: int, expires_hours: int = 24 * 7) -> Optional[str]:
        """
        创建用户会话
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    conn.commit()
                    logger.info(f"删除用户会话成功: 用户ID {user_id}, 删除数量: {deleted_count}")
                    return True
        except Exception as e:
    ...
    def start_weekly_cleanup(self):
        ...
        self._stop_cleanup = False
        self._cleanup_thread = threading.Thread(target=self._weekly_cleanup_worker, daemon=True)
        ...
        while not self._stop_cleanup:
            try:
                ...
                if now.weekday() == 0 and now.hour == 0 and now.minute == 0:
                    ...
                    logger.info(f"每周清理完成: 过期 {expired_count} 个会话，清理 {cleaned_count} 个过期会话")
    ...
    def force_weekly_cleanup(self) -> tuple[int, int]:
        ...
        logger.info(f"手动清理完成: 过期 {expired_count} 个会话，清理 {cleaned_count} 个过期会话")
