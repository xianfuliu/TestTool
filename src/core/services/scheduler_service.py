...
class UnifiedSchedulerService:
    ...
    def stop_service(self):
        ...
        # 停止服务主循环线程
        if (
            hasattr(self, "service_loop_thread")
            and self.service_loop_thread
            and self.service_loop_thread.is_alive()
        ):
    ...
    def run_service_loop(self):
        ...
        if wait_seconds > 0:
            logger.info(
                f"当前时间: {current_time}, 等待 {wait_seconds} 秒到整分钟再开始运行..."
            )
        ...
        if wait_seconds > 30:  # 如果等待时间超过30秒，在等待期间检查续期
            ...
            logger.info(
                f"等待期间续期检查 - 服务启动时间: {time_since_start:.0f}秒, 需要: 120秒"
            )
        ...
        while self.running:
            ...
            try:
                ...
                logger.info(
                    f"服务持续运行中 - 检查次数: {self.execution_count}, 错误次数: {self.error_count}, 运行时长: {uptime:.0f}秒"
                )

                # 定期续期分布式锁（测试用：每2分钟续期一次）
                current_time = datetime.now()
                time_since_last_renewal = (
                    current_time - last_lock_renewal
                ).total_seconds()

                # 调试日志：记录续期检查情况（每30秒记录一次）
                if iteration % 6 == 0:  # 每30秒记录一次（6次循环 * 5秒 = 30秒）
                    logger.info(
                        f"续期检查 - 循环次数: {iteration}, 距离上次续期: {time_since_last_renewal:.0f}秒, 需要: 120秒"
                    )

                if time_since_last_renewal >= 120:  # 测试用：2分钟
                    logger.info(
                        f"触发分布式锁续期 - 距离上次续期: {time_since_last_renewal:.0f}秒"
                    )
                ...
                self.error_count += 1
                logger.error(
                    f"第 {iteration} 次检查异常 (错误#{self.error_count}): {str(e)}"
                )
    ...
    def _check_resource_limits(self):
        ...
        try:
            ...
            if (
            ...
            ).total_seconds() >= self.memory_check_interval:
                ...
                if PSUTIL_AVAILABLE:
                    ...
                    # 基于线程数量和任务队列长度估算资源使用
                    memory_usage_mb = (
                        len(self.thread_pool) * 10 + len(self.pending_tasks) * 2
                    )
                    cpu_percent = min(
                        100, len(self.thread_pool) * 5 + len(self.pending_tasks)
                    )
                ...
                if memory_usage_mb > self.max_memory_mb:
                    logger.warning(
                        f"内存使用超过限制: {memory_usage_mb:.1f}MB > {self.max_memory_mb}MB"
                    )
                    self._handle_memory_overflow()

                if cpu_percent > self.max_cpu_percent:
                    logger.warning(
                        f"CPU使用率超过限制: {cpu_percent:.1f}% > {self.max_cpu_percent}%"
                    )
                ...
                if len(self.cpu_usage_history) > self.max_history_size:
                    self.cpu_usage_history = self.cpu_usage_history[
                        -self.max_history_size :
                    ]

                # 每5分钟记录一次资源使用情况
                if current_time.minute % 5 == 0 and current_time.second < 10:
                    avg_memory = sum(self.memory_usage_history[-10:]) / min(
                        10, len(self.memory_usage_history)
                    )
                    avg_cpu = sum(self.cpu_usage_history[-10:]) / min(
                        10, len(self.cpu_usage_history)
                    )
                    logger.info(
                        f"资源监控 - 内存: {avg_memory:.1f}MB, CPU: {avg_cpu:.1f}%, 活跃线程: {len(self.thread_pool)}"
                    )
    ...
    def _handle_memory_overflow(self):
        ...
        try:
            ...
            if len(self.pending_tasks) > self.max_pending_tasks // 2:
                logger.warning(
                    f"清理待处理队列，从 {len(self.pending_tasks)} 个任务中移除一半"
                )
    ...
    def _handle_cpu_overload(self):
        ...
        try:
            ...
            logger.info(
                f"临时调整检查间隔: {original_interval}秒 -> {self.check_interval}秒"
            )
    ...
    def _check_task_timeouts(self):
        ...
        try:
            ...
            # 定期检查任务超时
            if (
                current_time - self.last_timeout_check
            ).total_seconds() >= self.task_timeout_check_interval:
                ...
                if timeout_tasks:
                    ...
                    for scheduler_id, execution_time in timeout_tasks:
                        logger.warning(
                            f"  任务 {scheduler_id} 已执行 {execution_time:.1f}秒 > {self.max_task_execution_time}秒"
                        )
    ...
    def _cleanup_threads(self):
        ...
        if len(self.thread_pool) > 0 or len(self.pending_tasks) > 0:
            logger.info(
                f"当前活跃线程数: {len(self.thread_pool)}, 活跃调度数: {len(self.active_schedulers)}, 待处理任务数: {len(self.pending_tasks)}"
            )
    ...
    def _check_and_execute_schedulers(self):
        ...
        try:
            ...
            logger.info(
                f"检查时间: {current_time}, 启用的调度数量: {len(enabled_schedulers)}"
            )
            ...
            for scheduler in enabled_schedulers:
                ...
                if not cron_expression:
                    logger.warning(
                        f"调度 {scheduler_name} (ID: {scheduler_id}) 没有Cron表达式，跳过"
                    )
                    continue
                ...
                if scheduler_id in self.active_schedulers:
                    ...
                    if thread and thread.is_alive():
                        logger.warning(
                            f"调度 {scheduler_name} (ID: {scheduler_id}) 正在执行中，跳过本次执行"
                        )
                        continue
                ...
                if should_execute:
                    ...
                    if len(self.thread_pool) >= self.max_threads:
                        ...
                        if len(self.pending_tasks) < self.max_pending_tasks:
                            ...
                            logger.warning(
                                f"线程池已满，调度 {scheduler_name} 加入待处理队列，当前队列长度: {len(self.pending_tasks)}"
                            )
                        else:
                            logger.error(
                                f"线程池和待处理队列均已满，跳过调度 {scheduler_name}"
                            )
                        continue
    ...
    def _check_missed_schedulers(
        self, enabled_schedulers: List[Dict], current_time: datetime
    ):
        """检查过去一段时间内可能错过的调度任务
            ...
            for scheduler in enabled_schedulers:
                ...
                while check_time < current_time and not found_missed:
                    ...
                    if next_run_at_check_time and next_run_at_check_time < current_time:
                        # 找到错过的执行时间
                        time_diff = (
                            current_time - next_run_at_check_time
                        ).total_seconds()
                        if time_diff <= check_back_seconds:  # 只处理最近错过的任务
                            logger.warning(
                                f"发现错过的调度: {scheduler_name} (ID: {scheduler_id}), 应执行时间: {next_run_at_check_time}, 已错过: {time_diff:.1f}秒"
                            )

                            # 立即执行错过的任务
                            self._execute_missed_scheduler(
        ...
        try:
            ...
            if len(self.thread_pool) >= self.max_threads:
                ...
                if len(self.pending_tasks) < self.max_pending_tasks:
                    ...
                    logger.warning(
                        f"线程池已满，错过的调度 {scheduler_name} 加入待处理队列"
                    )
                else:
                    logger.error(
                        f"线程池和待处理队列均已满，无法执行错过的调度 {scheduler_name}"
                    )
                return
    ...
    def _execute_missed_scheduler_thread(
        self, scheduler_data: Dict, missed_time: datetime
    ):
        """执行错过调度的线程函数
        ...
        try:
            ...
            logger.info(
                f"{current_time} [INFO] 🔍 开始执行错过的调度: {scheduler_name} (ID: {scheduler_id})"
            )
    ...
    def _execute_scheduler_thread(self, scheduler_data):
        ...
        try:
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] 🔍 开始执行调度: {scheduler_name} (ID: {scheduler_id})"
            )
            ...
            if not case_ids:
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.warning(
                    f"{current_time} [WARN] ⚠️ 调度 '{scheduler_name}' 中没有配置测试用例"
                )
                return

            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] 📋 调度 {scheduler_name} 包含 {len(case_ids)} 个测试用例"
            )
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] 📄 创建测试报告成功，报告ID: {report_id}"
            )
            ...
            for i, case_id in enumerate(case_ids):
                try:
                    ...
                    if not case_data:
                        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        logger.warning(
                            f"{current_time} [WARN] ⚠️ 测试用例ID {case_id} 不存在，跳过执行"
                        )
                        continue
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(
                        f"{current_time} [INFO] 🚀 执行测试用例 [{i+1}/{total_count}]: {case_name} (ID: {case_id})"
                    )

                    # 使用TestCaseExecutor工具类执行测试用例，不生成独立报告
                    # 创建新的TestCaseExecutor实例，传递正确的项目ID和执行模式
                    from src.utils.interface_utils.test_case_executor import (
                        TestCaseExecutor,
                    )
                    ...
                    if execution_result.get("success"):
                        ...
                        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        logger.info(
                            f"{current_time} [SUCCESS] ✅ 测试用例 {case_name} (ID: {case_id}) 执行成功"
                        )
                        case_results.append(
                            {
                                "case_id": case_id,
                        ...
                        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        logger.warning(
                            f"{current_time} [FAILURE] ❌ 测试用例 {case_name} (ID: {case_id}) 执行失败"
                        )
                        case_results.append(
                            {
                                "case_id": case_id,
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.error(
                        f"{current_time} [ERROR] 💥 执行测试用例ID {case_id} 失败: {str(e)}"
                    )
                    case_results.append(
                        {
                            "case_id": case_id,
            ...
            if success_count == total_count:
                logger.info(
                    f"{current_time} [SUCCESS] 🎉 调度 {scheduler_name} 执行完成 - 成功: {success_count}/{total_count}, 耗时: {execution_duration:.2f}秒"
                )
            else:
                logger.warning(
                    f"{current_time} [FAILURE] ⚠️ 调度 {scheduler_name} 执行完成 - 成功: {success_count}/{total_count}, 耗时: {execution_duration:.2f}秒"
                )
            ...
            try:
                ...
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.info(
                    f"{current_time} [INFO] 📊 测试报告已更新，统计信息: 用例{passed_cases}/{total_cases}通过"
                )
            ...
            try:
                ...
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.info(
                    f"{current_time} [INFO] ⏰ 调度 {scheduler_name} 执行时间已更新"
                )
            except Exception as e:
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.error(
                    f"{current_time} [ERROR] 💥 更新调度执行时间失败: {str(e)}"
                )
            ...
            if notify_emails and email_enabled:
                try:
                    self._send_test_report_email(
                        notify_emails, case_results, scheduler_name, execution_duration
                    )
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(
                        f"{current_time} [INFO] 📧 测试报告邮件发送成功，收件人: {', '.join(notify_emails)}"
                    )
                except Exception as e:
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.error(
                        f"{current_time} [ERROR] 💥 发送测试报告邮件失败: {str(e)}"
                    )
            ...
            if wechat_webhook and wechat_enabled:
                try:
                    self._send_wechat_notification(
                        wechat_webhook, case_results, scheduler_name, execution_duration
                    )
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.error(
                        f"{current_time} [ERROR] 💥 发送企业微信通知失败: {str(e)}"
                    )
    ...
    def execute_scheduler(self, scheduler_id):
        ...
        try:
            ...
            if scheduler_id in self.active_schedulers:
                ...
                if thread and thread.is_alive():
                    logger.warning(
                        f"调度 '{scheduler_data['name']}' 正在执行中，跳过本次执行"
                    )
                    return False
    ...
    def check_scheduler_name_exists(
        self, name: str, exclude_scheduler_id: int = None
    ) -> bool:
        """检查调度名称是否已存在
    ...
    def get_schedulers_with_pagination(
        self, page: int = 1, page_size: int = 10, project_id: int = None
    ) -> tuple:
        """获取分页调度列表
    ...
    def _send_test_report_email(
        ...
        try:
            ...
            # 生成测试报告数据
            report_data = self._generate_email_report_data(
                case_results, scheduler_name, execution_duration
            )

            # 发送邮件
            return email_service.send_test_report_email(
                recipients, report_data, 0, scheduler_name
            )
    ...
    def _send_wechat_notification(
        ...
        try:
            # 生成测试报告数据
            report_data = self._generate_wechat_report_data(
                case_results, scheduler_name, execution_duration
            )
    ...
    def _generate_wechat_report_data(
        self, case_results: list, scheduler_name: str, execution_duration: float
    ) -> dict:
        """生成企业微信报告数据"""
        total_cases = len(case_results)
        success_cases = sum(
            1 for result in case_results if result.get("success", False)
        )
...
💡 执行结果: {report_data['status']}"""
    ...
    def _send_wechat_webhook(self, webhook_url: str, message: dict) -> bool:
        ...
        try:
            ...
            response = requests.post(
                webhook_url, data=json.dumps(message), headers=headers, timeout=10
            )
    ...
    def _generate_email_report_data(
        self, case_results: list, scheduler_name: str, execution_duration: float
    ) -> dict:
        """生成邮件报告数据"""
        total_cases = len(case_results)
                ...
                with conn.cursor() as cursor:
                    ...
                    try:
                        ...
                        if existing_lock:
                            # 已经有其他实例持有锁
                            logger.debug(
                                f"检测到已有调度服务实例在运行，实例ID: {existing_lock['instance_id']}"
                            )
                            conn.rollback()
                            return False
    ...
    def _start_service_loop(self):
        ...
        try:
            ...
            self.service_loop_thread = threading.Thread(
                target=self.run_service_loop, name="SchedulerServiceLoop"
            )
