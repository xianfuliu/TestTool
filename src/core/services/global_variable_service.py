...
class GlobalVariableService:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
                        SELECT id, name, value, variable_type, description, 
    ...
    def get_global_variable_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        """根据名称获取全局变量"""
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
    ...
    def create_global_variable(self, data: Dict[str, Any]) -> int:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    # 检查变量名是否已存在
                    cursor.execute("SELECT id FROM global_variables WHERE name = %s", (data['name'],))
                    if cursor.fetchone():
                        raise ValueError(f"全局变量 '{data['name']}' 已存在")
                    
                    # 处理值类型转换
                    value = self._convert_value_to_string(data['value'], data.get('variable_type', 'string'))
                    
                    cursor.execute("""
                        INSERT INTO global_variables (name, value, variable_type, description, created_by)
                        VALUES (%s, %s, %s, %s, %s)
                    """, (
                        data['name'],
                        value,
    ...
    def update_global_variable(self, var_id: int, data: Dict[str, Any]) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    # 检查变量名是否与其他变量冲突
                    cursor.execute("SELECT id FROM global_variables WHERE name = %s AND id != %s", 
                                 (data['name'], var_id))
                    if cursor.fetchone():
                        raise ValueError(f"全局变量 '{data['name']}' 已存在")
                    ...
                    cursor.execute("""
                        ...
                        SET name = %s, value = %s, variable_type = %s, description = %s
                        WHERE id = %s
                    """, (
                        data['name'],
                        value,
    ...
    def delete_global_variable_by_name(self, name: str) -> bool:
        """根据名称删除全局变量"""
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("DELETE FROM global_variables WHERE name = %s", (name,))
                    conn.commit()
                    return cursor.rowcount > 0
        except Exception as e:
            print(f"删除全局变量失败: {e}")
    ...
    def sync_to_variable_manager(self, variable_manager) -> bool:
        """将数据库中的全局变量同步到变量管理器"""
        try:
            global_vars = self.get_all_global_variables()
            var_dict = {}
            ...
            print(f"同步全局变量到变量管理器失败: {e}")
            return False

    def sync_from_variable_manager(self, variable_manager) -> bool:
        """将变量管理器中的全局变量同步到数据库"""
        try:
            # 获取当前内存中的全局变量
            ...
            # 获取数据库中的全局变量
            db_vars = self.get_all_global_variables()
