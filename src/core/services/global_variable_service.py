...
class GlobalVariableService:
    ...
    def get_global_variables_by_project(
    ...
    def get_global_variable_by_name(
        self, name: str, project_id: int = 0
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # 检查变量名是否已存在（同一项目内）
                    cursor.execute(
                        "SELECT id FROM global_variables WHERE project_id = %s AND name = %s",
                        (project_id, data["name"]),
                    )
                    if cursor.fetchone():
                        raise ValueError(
                            f"项目 {project_id} 中全局变量 '{data['name']}' 已存在"
                        )

                    # 处理值类型转换
                    value = self._convert_value_to_string(
                        data["value"], data.get("variable_type", "string")
                    )
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        "SELECT id FROM global_variables WHERE project_id = %s AND name = %s AND id != %s",
    ...
    def delete_global_variable(self, var_id: int) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "DELETE FROM global_variables WHERE id = %s", (var_id,)
                    )
                    conn.commit()
                    return cursor.rowcount > 0
        except Exception as e:
    ...
    def delete_global_variable_by_name(self, name: str, project_id: int = 0) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "DELETE FROM global_variables WHERE project_id = %s AND name = %s",
                        (project_id, name),
                    )
                    conn.commit()
                    return cursor.rowcount > 0
