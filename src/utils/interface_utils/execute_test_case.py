...
class ExecuteTestCase:
    ...
    def execute_test_case_unified(
        ...
        try:
            ...
            logger.info(
                f"{current_time} [INFO] 🔍 开始执行测试用例: {case_name} (ID: {case_id})"
            )
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] 📋 测试用例 {case_name} 包含 {len(steps)} 个步骤"
            )
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] ✅ 启用的步骤数量: {len(enabled_steps)}"
            )
            ...
            try:
                ...
                service.sync_to_variable_manager(
                    variable_manager, case_data.get("project_id", 0)
                )
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.info(
                    f"{current_time} [INFO] 📊 已加载项目 {case_data.get('project_id', 0)} 的全局变量"
                )
            ...
            for i, step in enumerate(steps):
                ...
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.info(
                    f"{current_time} [INFO] 🚀 执行步骤 [{i+1}/{len(enabled_steps)}]: {step_name}"
                )

                # 执行单个步骤（使用统一的步骤执行逻辑）
                step_result = self._execute_test_step_unified(
                    step, i, case_data, variable_manager
                )
                ...
                if generate_report or parent_report_id:
                    try:
                        # 延迟导入避免循环导入
                        from src.core.services.test_report_service import (
                            TestReportService,
                        )
                        ...
                        step_result_data = {
                            ...
                            "post_processing_result": step_result.get(
                                "post_processing"
                            ),
                        ...
                        if parent_report_id:
                            ...
                            report_service.save_step_result(
                                parent_report_id, step_result_data
                            )
                            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.debug(
                                f"{current_time} [DEBUG] 💾 步骤执行结果已保存到父报告: 步骤{step_result_data.get('step_order')}"
                            )
                        else:
                            ...
                            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logger.debug(
                                f"{current_time} [DEBUG] 💾 步骤执行结果已暂存: {step.get('name', f'步骤{i+1}')}"
                            )
                    except Exception as e:
                        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        logger.warning(
                            f"{current_time} [WARN] ⚠️ 保存步骤执行结果失败: {str(e)}"
                        )
                ...
                if step_result.get("status") == "error":
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.warning(
                        f"{current_time} [WARN] ❌ 步骤 {step_name} 执行失败，继续执行后续步骤"
                    )
                elif step_result.get("status") == "failure":
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.warning(
            ...
            if generate_report and report_data:
                ...
                report_data["duration"] = (
                    report_data["end_time"] - report_data["start_time"]
                ).total_seconds()
                ...
                try:
                    ...
                    report_data["report_id"] = (
                        report_id  # 保存report_id用于后续步骤结果保存
                    )
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(
                        f"{current_time} [INFO] 📄 测试报告已保存，ID: {report_id}"
                    )
                    ...
                    if step_results_data:
                        for step_result_data in step_results_data:
                            try:
                                ...
                                report_service.save_step_result(
                                    report_id, step_result_data
                                )
                                current_time = datetime.now().strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                )
                                logger.debug(
                                    f"{current_time} [DEBUG] 💾 步骤执行结果已保存到数据库: 步骤{step_result_data.get('step_order')}"
                                )
                            except Exception as e:
                                current_time = datetime.now().strftime(
                                    "%Y-%m-%d %H:%M:%S"
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.warning(
                        f"{current_time} [WARN] ⚠️ 保存测试报告失败: {str(e)}"
                    )

            # 记录用例执行统计信息
            success_count = sum(
                1 for result in step_results if result.get("status") == "success"
            )
            failure_count = sum(
                1 for result in step_results if result.get("status") == "failure"
            )
            error_count = sum(
                1 for result in step_results if result.get("status") == "error"
            )

            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] 🎉 测试用例 {case_name} 执行完成: 成功 {success_count}/{len(enabled_steps)} 个步骤"
            )
            ...
            if generate_report and report_data:
                try:
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info(
                        f"{current_time} [INFO] 📊 测试报告用例统计已更新: 总用例1, 通过{update_data['passed_cases']}, 失败{update_data['failed_cases']}, 错误{update_data['error_cases']}"
                    )

                except Exception as e:
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.warning(
                        f"{current_time} [WARN] ⚠️ 更新测试报告步骤统计失败: {str(e)}"
                    )
            ...
            if generate_report:
                ...
                try:
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.warning(
                        f"{current_time} [WARN] ⚠️ 保存错误报告失败: {str(e)}"
                    )
    ...
    def _execute_test_step_unified(
        self,
        ...
        try:
            ...
            logger.info(
                f"{current_time} [INFO] 🔍 开始执行步骤 [{step_index+1}]: {step_name}"
            )
            ...
            if not api_template_id:
                ...
                return {
                    ...
                    "variables_snapshot": (
                        variables_snapshot if "variables_snapshot" in locals() else {}
                    ),
            ...
            logger.info(
                f"{current_time} [INFO] 🌐 执行步骤 {step_name} 的HTTP请求 [{api_template_name}]: {api_data.get('method', 'GET')} {api_data.get('url_path', '')}"
            )
            ...
            if post_processing:
                ...
                post_processing_result = self._execute_post_processing(
                    post_processing, response_data, step_index
                )
                # 将后置处理的执行日志合并到主步骤日志中
                if (
                    post_processing_result
                    and "execution_logs" in post_processing_result
                ):
            ...
            if assertions:
                ...
                assertions_result = self._execute_assertions(
                    assertions, response_data, step_index
                )
                # 将断言的执行日志合并到主步骤日志中
            ...
            logger.info(
                f"{current_time} [SUCCESS] {status_icon} 步骤 {step_name} 执行完成，状态: {status}"
            )
            ...
            return {
                "step_id": step_data.get("id"),
                "step_name": (
                    step_name if "step_name" in locals() else f"步骤{step_index+1}"
                ),
                "step_order": step_index,
                "status": "error",
                "error_message": str(e),
                "start_time": (
                    step_start_time if "step_start_time" in locals() else datetime.now()
                ),
                "end_time": datetime.now(),
                "execution_time": 0,
                "execution_logs": execution_logs,
    ...
    def _execute_pre_processing(
        ...
        try:
            ...
            logger.info(
                f"{current_time} [INFO] 🔧 开始执行前置处理，工具数量: {tool_count}"
            )
            ...
            for tool_id, tool_config in pre_processing.items():
                ...
                try:
                    if tool_type == "http_request":
                        ...
                        tool_result["result"] = self._execute_http_request_tool(
                            config, step_index
                        )
                        ...
                        tool_result["result"] = self._execute_sql_tool(
                            config, step_index
                        )
                    else:
                        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        ...
                        logger.warning(
                            f"{current_time} [WARN] 🔧 未知的前置处理工具类型: {tool_type}"
                        )
                    ...
                    logger.error(
                        f"{current_time} [ERROR] 🔧 执行前置处理工具 {tool_type} 失败: {str(e)}"
                    )
            ...
            if variables:
                ...
                logger.info(
                    f"{current_time} [INFO] 🔧 设置局部变量: {len(variables)} 个"
                )
            ...
            logger.info(
                f"{current_time} [INFO] 🔧 前置处理完成，共执行 {executed_tool_count} 个工具"
            )
    ...
    def _execute_post_processing(
        ...
        try:
            ...
            if extractors:
                ...
                logger.info(
                    f"{current_time} [INFO] 🔧 执行变量提取，提取器数量: {len(extractors)}"
                )
                # 这里可以集成变量提取功能
    ...
    def _execute_assertions(
        ...
        try:
            ...
            for i, assertion in enumerate(assertions_config):
                ...
                try:
                    ...
                    if assertion_result["passed"]:
                        ...
                        logger.warning(
                            f"{current_time} [FAILURE] ❌ 断言{i+1} 验证失败"
                        )
                    ...
                    logger.error(
                        f"{current_time} [ERROR] ❌ 断言{i+1}执行失败: {str(e)}"
                    )
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            complete_msg = (
                f"✅ 断言验证完成: 通过{result['passed_count']}/{result['total_count']}"
            )
            result["execution_logs"].append(
                {
                    "timestamp": datetime.now().isoformat(),
            ...
            logger.info(
                f"{current_time} [INFO] ✅ 断言验证完成: 通过{result['passed_count']}/{result['total_count']}"
            )
    ...
    def _execute_http_request_tool(self, config: Dict[str, Any], step_index: int):
        ...
        try:
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.info(
                f"{current_time} [INFO] 🔧 开始执行HTTP请求工具，配置: {config}"
            )
            ...
            if not url:
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.error(
                    f"{current_time} [ERROR] 🔧 HTTP请求工具配置错误: URL不能为空"
                )
                return
    ...
    def _execute_sql_tool(self, config: Dict[str, Any], step_index: int):
        ...
        try:
            ...
            if not sql:
                current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                logger.error(
                    f"{current_time} [ERROR] 🔧 SQL工具配置错误: SQL语句不能为空"
                )
                return
            ...
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if api_template:
                        # 处理JSON字段
                        headers = (
                            json.loads(api_template["headers"])
                            if api_template["headers"]
                            else {}
                        )
                        body = (
                            json.loads(api_template["body"])
                            if api_template["body"]
                            else {}
                        )
