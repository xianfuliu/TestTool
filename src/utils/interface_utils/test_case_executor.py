...
class TestCaseExecutor:
    ...
    def _store_step_logs_to_database(
        self,
        step_id: int,
        step_name: str,
        status: str,
        ...
        if self.debug_mode:
            self._log_message(
                "DEBUG",
                f"调试模式，跳过步骤 '{step_name}' 日志存储到数据库",
                step_index,
            )
            return
        ...
        try:
            ...
            step_result_data = {
                ...
                "case_id": case_id,
                "step_id": (
                    step_id if step_id is not None else 0
                ),  # 如果step_id为None，使用0作为默认值
                "step_order": (
                    step_order or (step_index + 1) if step_index is not None else 1
                ),
                "status": status,
            ...
            if success:
                self._log_message(
                    "INFO", f"步骤 '{step_name}' 执行日志已存储到数据库", step_index
                )
            else:
                self._log_message(
                    "ERROR", f"存储步骤执行日志失败: 保存步骤结果返回False", step_index
                )
    ...
    def _init_variable_manager(self):
        ...
        try:
            ...
            self._log_message(
                "WARNING", f"加载项目 {self.project_id} 的全局变量失败: {str(e)}"
            )
        ...
        try:
            ...
            for step in steps:
                ...
                if not isinstance(step_order, int) or step_order <= 0:
                    ...
                    logging.warning(
                        f"检测到无效步骤序号{step.get('step_order')}，已修正为: {step_order}"
                    )
                ...
                step_name = (
                    step.get("api_name")
                    or step.get("api_template", {}).get("name")
                    or step.get("name", f"步骤{step_order}")
                )
                ...
                if self.step_started_callback:
                    try:
                        ...
                        self._log_message(
                            "ERROR", f"步骤开始回调执行异常: {str(e)}", step_index
                        )

                self._log_message(
                    "INFO", f"开始执行步骤 {step_order}: {step_name}", step_index
                )
                ...
                if self.step_finished_callback:
                    try:
                        ...
                        self._log_message(
                            "ERROR", f"步骤完成回调执行异常: {str(e)}", step_index
                        )

                # 如果是调度模式且有调度ID和报告ID，存储步骤日志到数据库
                if (
                    self.execution_mode == "scheduler"
                    and scheduler_id
                    and parent_report_id
                ):
                ...
                if not step_result.get("success", False) and stop_on_failure:
                    self._log_message(
                        "WARNING", "步骤执行失败，停止执行后续步骤", step_index
                    )
                    break
            ...
            self._log_message(
                "INFO", f"步骤统计: 成功 {success_steps}/{len(enabled_steps)}"
            )
    ...
    def _execute_step(
        self, step_data: Dict[str, Any], step_index: int, case_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            step_name = (
                step_data.get("api_name")
                or step_data.get("api_template", {}).get("name")
                or step_data.get("name", f"步骤{step_order}")
            )
            ...
            pre_processing_result = self._execute_pre_processing(
                pre_processing, step_index
            )
            ...
            if pre_processing_result.get("errors"):
                ...
                return self._create_step_error_result(
                    step_data, step_index, step_start_time, error_msg
                )
            ...
            if api_template_id:
                ...
                try:
                    response = self.execute_api_request(
                        step_data, step_index, case_data
                    )
                except Exception as e:
                    self._log_message(
                        "ERROR", f"接口请求执行异常: {str(e)}", step_index
                    )
                    response = {
                        "success": False,
                        "error": str(e),
            ...
            if response.get("success"):
                ...
                if post_processing:
                    ...
                    post_processing_result = self._execute_post_processing(
                        post_processing, response, step_index
                    )
            ...
            if response.get("success"):
                ...
                if assertions:
                    ...
                    assertions_result = self._execute_assertions(
                        assertions, response, step_index
                    )
                response["assertions_result"] = assertions_result

            # 5. 判断步骤状态（与tabbed_case_editor.py保持一致）
            step_success = response.get("success", False) and assertions_result.get(
                "passed", True
            )
            ...
            self._log_message(
                log_level,
                f"步骤 {step_order} 执行完成: {status}, 耗时: {execution_time:.2f}秒",
                step_index,
            )
            ...
            self._log_message("ERROR", error_msg, step_index)
            self._log_message(
                "DEBUG", f"错误详情: {traceback.format_exc()}", step_index
            )
            return self._create_step_error_result(
                step_data, step_index, step_start_time, error_msg
            )

    def _get_api_template(
        self, step_data: Dict[str, Any], step_index: int = None
    ...
    def _prepare_request_data(
        ...
        # 替换变量
        url = self.variable_manager.replace_variables(
            api_template["url_path"], all_variables
        )
        headers = self.variable_manager.replace_variables_in_dict(
            api_template.get("headers", {}), all_variables
        )
        params = self.variable_manager.replace_variables_in_dict(
            api_template.get("params", {}), all_variables
        )
        body = self.variable_manager.replace_variables_in_dict(
            api_template.get("body", {}), all_variables
        )
    ...
    def _execute_pre_processing(
        self, pre_processing: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        """执行前置处理"""
        result = {"tools_executed": [], "variables_set": {}, "errors": []}
        ...
        sorted_tools = sorted(
            pre_processing.items(), key=lambda x: x[1].get("priority", 0)
        )
        ...
        for tool_id, tool_config in sorted_tools:
            ...
            try:
                ...
                # 执行前置处理工具
                tool_result = self._execute_pre_processing_tool(
                    tool_type, tool_config, step_index
                )
                if tool_result:
                    ...
                    if "variables_set" in tool_result:
                        ...
                        for var_name, var_value in tool_result["variables_set"].items():
                            self.variable_manager.set_local_variable(
                                var_name, var_value
                            )
        ...
        self._log_message(
            "INFO", f"前置处理完成，共执行 {executed_tool_count} 个工具", step_index
        )

        return result

    def _execute_pre_processing_tool(
        self, tool_type: str, tool_config: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        """执行具体的前置处理工具"""
        result = {"variables_set": {}, "success": True}
        ...
        if tool_type == "http_request":
            ...
            self._log_message(
                "WARNING", f"未知的前置处理工具类型: {tool_type}", step_index
            )
    ...
    def execute_api_request(
        self, step: Dict[str, Any], step_index: int, case_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """执行接口请求（用于定时调度执行）
        ...
        try:
            ...
            if not api_template:
                self._log_message(
                    "ERROR", f"接口模板不存在: {api_template_id}", step_index
                )
                return {
                    "success": False,
            ...
            request_data = self._prepare_request_data(
                api_template, step_variables, step_index
            )
            ...
            step_name = (
                step.get("api_name")
                or step.get("api_template", {}).get("name")
                or step.get("name", f"步骤{step_index+1}")
            )
            ...
            self._log_message(
                "INFO",
                f"执行步骤 {step_name} 的HTTP请求 - {api_template_name}: {request_data['method']} {request_data['url']}",
                step_index,
            )
            self._log_message(
                "DEBUG",
            ...
            try:
                ...
                if case_data is None:
                    case_data = {}
                global_enable_encryption = case_data.get(
                    "enable_encryption",
                    self.environment_config.get("enable_encryption", False),
                )
                global_encrypt_url = case_data.get(
                    "encrypt_url", self.environment_config.get("encrypt_url", "")
                )
                global_decrypt_url = case_data.get(
                    "decrypt_url", self.environment_config.get("decrypt_url", "")
                )

                # 判断是否使用加解密：步骤开启且全局配置完整
                use_encryption = (
                    enable_encryption
                    and global_enable_encryption
                    and global_encrypt_url
                    and global_decrypt_url
                )

                if use_encryption:
                    # 步骤启用加解密且全局配置完整
                    self._log_message(
                        "INFO", "步骤启用加解密功能，使用RequestEngine处理", step_index
                    )
                else:
                    # 不使用加解密的情况
                    if enable_encryption and not (
                        global_encrypt_url and global_decrypt_url
                    ):
                        # 步骤启用但全局配置不完整
                        self._log_message(
                            "WARNING",
                            "步骤启用加解密但全局配置不完整，使用普通请求",
                            step_index,
                        )
                        ...
                        self._log_message(
                            "WARNING",
                            "步骤启用加解密但全局未启用，使用普通请求",
                            step_index,
                        )
                    else:
                        # 步骤关闭加解密
                        self._log_message(
                            "INFO", "步骤关闭加解密功能，使用普通请求", step_index
                        )
                ...
                if use_encryption:
                    ...
                    # 准备请求数据（用于调试信息）
                    request_data = self._prepare_request_data(
                        api_template, step.get("variables", {}), step_index
                    )
                    ...
                    # 使用普通的RequestEngine
                    response = self.request_engine.execute_request(
                        api_data, step.get("variables", {})
                    )
                ...
                if response["success"]:
                    ...
                    if response.get("decrypted_body"):
                        try:
                            ...
                            decrypted_json = json.loads(
                                response.get("decrypted_body", "")
                            )
                            self._log_message(
                                "DEBUG",
                                f"解密后的响应体:{json.dumps(decrypted_json, ensure_ascii=False)}",
                                step_index,
                            )
                        except:
                            # 如果解析失败，直接打印原始字符串
                            self._log_message(
                                "DEBUG",
                                f"解密后的响应体:{response.get('decrypted_body', '')}",
                                step_index,
                            )
                    else:
                        ...
                        try:
                            ...
                            self._log_message(
                                "DEBUG",
                                f"响应体: {json.dumps(response_json, ensure_ascii=False)}",
                                step_index,
                            )
                        except:
                            # 如果解析失败，直接打印原始文本
                            self._log_message(
                                "DEBUG", f"响应体: {response_text}", step_index
                            )
                else:
                    # 记录失败请求的详细信息到日志（单行格式）
                    self._log_message(
                    ...
                    if response.get("decrypted_body"):
                        ...
                        try:
                            ...
                            self._log_message(
                                "DEBUG",
                                f"失败响应体: {json.dumps(response_json, ensure_ascii=False)}",
                                step_index,
                            )
                        except:
                            # 如果解析失败，直接打印原始文本
                            self._log_message(
                                "DEBUG", f"失败响应体: {response_text}", step_index
                            )
                ...
                self._log_message(
                    "ERROR", f"异常堆栈: {traceback.format_exc()}", step_index
                )
                return {"success": False, "error": str(e), "status": "error"}
        except Exception as e:
            self._log_message("ERROR", f"执行接口请求外层异常: {str(e)}", step_index)
            self._log_message(
                "ERROR", f"外层异常堆栈: {traceback.format_exc()}", step_index
            )
            return {"success": False, "error": str(e), "status": "error"}

    def _execute_http_request_tool(
        self, config: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        """执行HTTP请求工具"""
        result = {"variables_set": {}, "success": True}
        ...
        try:
            ...
            self._log_message(
                "INFO",
                f"HTTP请求工具配置 - 名称: {name}, 方法: {method}, URL: {url}, 超时: {timeout}",
                step_index,
            )
            self._log_message("DEBUG", f"HTTP请求工具配置 - 请求体: {body}", step_index)
            self._log_message(
                "DEBUG", f"HTTP请求工具配置 - 提取器: {extractors}", step_index
            )
            ...
            # 记录请求日志
            self._log_message(
                "INFO", f"前置处理器HTTP请求工具-{name}: {method} {url}", step_index
            )
            ...
            if response.get("success"):
                ...
                self._log_message(
                    "INFO",
                    f"前置处理器HTTP请求工具-{name}成功: 状态码 {status_code}",
                    step_index,
                )
                ...
                if extractors:
                    ...
                    for var_name, json_path in extractors.items():
                        try:
                            self._log_message(
                                "INFO",
                                f"提取变量 {var_name}，JSON路径: {json_path}",
                                step_index,
                            )
                            # 从响应中提取数据
                            value = self._simple_json_path_extract(
                                response_data, json_path
                            )
                            if value is not None:
                                ...
                                self._log_message(
                                    "DEBUG",
                                    f"变量管理器状态 - 局部变量: {self.variable_manager.local_variables}",
                                    step_index,
                                )
                                ...
                                if self.debug_mode:
                                    self._log_message(
                                        "DEBUG",
                                        f"变量管理器状态 - 全局变量: {self.variable_manager.global_variables}",
                                        step_index,
                                    )
                            else:
                                self._log_message(
                                    "WARNING",
                                    f"提取变量失败: {var_name}，JSON路径 {json_path} 未找到数据",
                                    step_index,
                                )
                                # 调试：检查响应数据结构
                                self._log_message(
                                    "DEBUG",
                                    f"响应数据结构: {response_data}",
                                    step_index,
                                )
                                self._log_message(
                                    "DEBUG",
                                    f"尝试提取路径 {json_path} 失败，检查响应数据格式",
                                    step_index,
                                )
                        except Exception as e:
                            self._log_message(
                                "ERROR",
                                f"提取变量异常: {var_name}，错误: {str(e)}",
                                step_index,
                            )
                else:
                ...
                self._log_message(
                    "ERROR", f"前置处理器HTTP请求失败: {error_msg}", step_index
                )
                result["success"] = False
                result["error"] = error_msg
    ...
    def _execute_sql_tool(
        ...
        try:
            ...
            self._log_message(
                "DEBUG", f"SQL工具配置 - 输出字段: {output_fields}", step_index
            )
            ...
            sql_worker = SQLWorker(
                "pre_processing_sql", database_config, processed_sql, all_variables
            )
            ...
            if sql_result["success"]:
                ...
                self._log_message(
                    "INFO",
                    f"前置处理器SQL工具 - {name} 执行成功: 返回 {len(data)} 行数据",
                    step_index,
                )
                ...
                if output_fields and data:
                    self._log_message(
                        "INFO",
                        f"开始提取变量，输出字段数量: {len(output_fields)}",
                        step_index,
                    )
                    ...
                    for field_config in output_fields:
                        ...
                        if field_name and field_name in first_row:
                            ...
                            self._log_message(
                                "DEBUG",
                                f"SQL工具 - {name} 提取变量后局部变量状态: {self.variable_manager.local_variables}",
                                step_index,
                            )
                        else:
                            self._log_message(
                                "WARNING",
                                f"提取变量失败: 字段 {field_name} 不存在或为空",
                                step_index,
                            )
                else:
                    self._log_message(
                        "DEBUG", "无输出字段或查询结果为空，跳过变量提取", step_index
                    )
            else:
                # SQL执行失败
                error_msg = sql_result.get("error", "未知错误")
                self._log_message(
                    "ERROR", f"前置处理器SQL执行失败: {error_msg}", step_index
                )
                result["success"] = False
                result["error"] = error_msg
    ...
    def _execute_post_processing(
        ...
        sorted_tools = sorted(
            post_processing.items(), key=lambda x: x[1].get("priority", 0)
        )
        ...
        for tool_id, tool_config in sorted_tools:
            ...
            try:
                # 执行后置处理工具
                tool_result = self._execute_post_processing_tool(
                    tool_type, tool_config, response_data, step_index
                )
                if tool_result:
                    result["tools_executed"].append(tool_id)
                    # 合并变量设置
    ...
    def _execute_post_processing_tool(
        self,
        tool_type: str,
        ...
        if tool_type == "parameter_extraction":
            ...
            self._log_message(
                "WARNING", f"未知的后置处理工具类型: {tool_type}", step_index
            )
            result["success"] = False
    ...
    def _execute_parameter_extraction(
        self, config: Dict[str, Any], response_data: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_message(
                "INFO", f"开始提取参数，提取器数量: {len(extractions)}", step_index
            )
            ...
            if decrypted_body:
                ...
                try:
                    ...
                    self._log_message(
                        "DEBUG", "使用解密后的响应体进行参数提取", step_index
                    )
                except:
                    # 如果解析失败，使用原始响应数据
                    response_data_for_extraction = response_data.get(
                        "response_data", {}
                    )
                    self._log_message(
                        "WARNING",
                        "解密后的响应体解析失败，使用原始响应数据",
                        step_index,
                    )
            else:
            ...
            for extraction in extractions:
                try:
                    ...
                    if not variable_name or not json_path:
                        self._log_message(
                            "WARNING", "参数提取器配置不完整，跳过", step_index
                        )
                        continue

                    self._log_message(
                        "INFO",
                        f"提取参数 {variable_name}，JSON路径: {json_path}",
                        step_index,
                    )

                    # 从响应体中提取数据
                    value = self._simple_json_path_extract(
                        response_data_for_extraction, json_path
                    )
                    ...
                    if value is not None:
                        ...
                        self._log_message(
                            "DEBUG",
                            f"参数提取后局部变量状态: {self.variable_manager.local_variables}",
                            step_index,
                        )
                    else:
                        self._log_message(
                            "WARNING",
                            f"提取参数失败: {variable_name}，JSON路径 {json_path} 未找到数据",
                            step_index,
                        )
                    ...
                    self._log_message(
                        "ERROR",
                        f"提取参数异常: {variable_name}，错误: {str(e)}",
                        step_index,
                    )
    ...
    def _execute_parameter_extractor(
        self, config: Dict[str, Any], response_data: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        """执行参数提取器（兼容性支持）"""
        result = {"variables_set": {}, "success": True}
        ...
        try:
            ...
            self._log_message(
                "INFO", f"开始提取参数，提取器数量: {len(extractors)}", step_index
            )
            ...
            for var_name, extractor_config in extractors.items():
                try:
                    ...
                    self._log_message(
                        "INFO",
                        f"提取参数 {var_name}，来源: {source}，JSON路径: {json_path}",
                        step_index,
                    )
                    ...
                    if source == "body":
                        ...
                        self._log_message(
                            "WARNING", f"未知的提取来源: {source}", step_index
                        )
                        continue
                    ...
                    if value is not None:
                        ...
                        self._log_message(
                            "SUCCESS", f"提取参数成功: {var_name} = {value}", step_index
                        )
                        ...
                        self._log_message(
                            "WARNING",
                            f"提取参数失败: {var_name}，未找到数据",
                            step_index,
                        )

                except Exception as e:
                    self._log_message(
                        "ERROR", f"提取参数异常: {var_name}，错误: {str(e)}", step_index
                    )
    ...
    def _execute_response_extractor(
        self, config: Dict[str, Any], response_data: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        """执行响应数据提取器"""
        result = {"variables_set": {}, "success": True}
        ...
        try:
            ...
            self._log_message(
                "INFO", f"开始提取响应数据，变量数量: {len(variables)}", step_index
            )
            ...
            for var_name, json_path in variables.items():
                try:
                    ...
                    if value is not None:
                        ...
                        self.variable_manager.set_local_variable(var_name, value)
                        self._log_message(
                            "SUCCESS", f"提取变量成功: {var_name} = {value}", step_index
                        )

                        # 打印局部变量状态
                        self._log_message(
                            "DEBUG",
                            f"响应数据提取后局部变量状态: {self.variable_manager.local_variables}",
                            step_index,
                        )
                    else:
                        self._log_message(
                    ...
                    self._log_message(
                        "ERROR", f"提取变量异常: {var_name}，错误: {str(e)}", step_index
                    )
    ...
    def _execute_assertions(
        self, assertions: Dict[str, Any], response_data: Dict[str, Any], step_index: int
    ) -> Dict[str, Any]:
        ...
        response_headers = response_data.get(
            "response_headers", response_data.get("headers", {})
        )
        response_time = response_data.get(
        ...
        if decrypted_body:
            ...
            isinstance(response_data.get("response_data", ""), str)
            and response_data.get("response_data", "").strip()
        ):
            # 如果没有解密后的响应体，但response_data是字符串，尝试解析为JSON
            try:
                response_data["response_data"] = json.loads(
        ...
        sorted_assertions = sorted(
            assertions.items(), key=lambda x: x[1].get("priority", 0)
        )
        ...
        for assertion_name, assertion_config in sorted_assertions:
            try:
                ...
                if "type" in assertion_config:
                    ...
                    if assertions_list:
                        ...
                        if assertion_type == "response_time" and assertions_list:
                            ...
                            config["time_value"] = (
                                float(assertions_list[0].get("expected"))
                                if assertions_list[0].get("expected")
                                else None
                            )
                ...
                # 执行断言
                assertion_result = self._execute_single_assertion_new(
                    assertion_type,
                    config,
                    response_data,
                    response_text,
                ...
                self._log_message(
                    "ERROR", f"断言 {assertion_name} 执行错误: {str(e)}", step_index
                )
        ...
        self._log_message(
            "INFO", f"断言结果: {passed_count}/{len(assertions)} 通过", step_index
        )
    ...
    def _execute_single_assertion_new(
        self,
        assertion_type: str,
        config: Dict[str, Any],
        response_data: Dict[str, Any],
        ...
        if assertion_type == "equal":
            ...
            if expected is not None:
                expected = self.replace_variables(
                    str(expected), step_result, step_index
                )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 期望值='{expected_display}' 实际值='{actual_display}' 比较结果:{'==' if result else '!='}",
                step_index,
            )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 期望包含='{expected_display}' 实际响应='{actual_display}' 结果:{result}",
                step_index,
            )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 期望不包含='{expected_display}' 实际响应='{actual_display}' 结果:{result}",
                step_index,
            )
            ...
            try:
                ...
                self._log_message(
                    "INFO",
                    f"断言 {assertion_name}: 正则匹配 '{pattern}' -> {result}",
                    step_index,
                )
                ...
                self._log_message(
                    "ERROR",
                    f"断言 {assertion_name}: 正则表达式错误: {str(e)}",
                    step_index,
                )
                return False
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 状态码 {actual_display} {'==' if result else '!='} {expected_display}",
                step_index,
            )
            ...
            actual = self._simple_json_path_extract(
                response_data.get("response_data", {}), path
            )
            result = expected == actual
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: {path} = {actual_display} {'==' if result else '!='} {expected_display}",
                step_index,
            )
            ...
            if expected_value is not None:
                expected_value = self.replace_variables(
                    str(expected_value), step_result, step_index
                )
            actual = response_time

            # 如果期望值为None，则使用0作为默认值进行比较
            expected_value_num = (
                float(expected_value) if expected_value is not None else 0
            )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 响应时间 {actual:.3f}s {comparison_symbol} {expected_display}s -> {result}",
                step_index,
            )
            ...
            if header_name is not None:
                header_name = self.replace_variables(
                    str(header_name), step_result, step_index
                )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 响应头包含 '{header_name}' -> {result}",
                step_index,
            )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: JSON Schema验证 -> {result}",
                step_index,
            )
            ...
            if not config.get("enabled", True):
                self._log_message(
                    "INFO", f"断言 {assertion_name}: 断言已禁用，视为通过", step_index
                )
            ...
            for i, assertion in enumerate(assertions_list):
                ...
                self._log_message(
                    "INFO",
                    f"断言 {assertion_name} 第{i+1}行: {field} {symbol} {expected_display} -> {result} (实际值: {actual_display})",
                    step_index,
                )
            ...
            self._log_message(
                "INFO",
                f"断言 {assertion_name}: 所有断言行执行完成，最终结果: {final_result}",
                step_index,
            )
            ...
            try:
                ...
                self._log_message(
                    "INFO",
                    f"断言 {assertion_name}: {field_name} {assertion_type} {expected_display} -> {result} (实际值: {actual_display})",
                    step_index,
                )
                ...
                self._log_message(
                    "ERROR",
                    f"断言 {assertion_name}: 数值转换错误 - 期望值: {expected}, 实际值: {actual}",
                    step_index,
                )
                return False
    ...
    def _execute_single_assertion(
        self, assertion_config: Dict[str, Any], response_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行单个断言（兼容旧格式）"""
        # 获取断言配置
        ...
        # 执行比较
        passed = self._execute_comparison(
            actual_value, expected_value, comparison_symbol
        )
    ...
    def _extract_field_value(
        self, response_data: Dict[str, Any], field_path: str
    ) -> Any:
        ...
        if decrypted_body:
            ...
            try:
                ...
                # 如果解析失败，使用原始响应数据
                response_data_json = response_data.get(
                    "response_data", response_data.get("body", {})
                )
                response_text = response_data.get(
                    "response_text", response_data.get("text", "")
                )
    ...
    def _extract_field_value_from_response(
        self, response_data: Dict[str, Any], field_path: str
    ) -> Any:
    ...
    def replace_variables(
        self, text: str, step_result: Dict[str, Any], step_index: int = None
    ) -> str:
        ...
        # 调试：记录替换前的文本
        self._log_message(
            "DEBUG", f"replace_variables: 替换前文本 = {text}", step_index
        )
        ...
        # 调试：记录匹配到的变量
        self._log_message(
            "DEBUG", f"replace_variables: 匹配到的变量 = {matches}", step_index
        )
        ...
        for var_name in matches:
            ...
            if hasattr(self, "variable_manager"):
                ...
                self._log_message(
                    "DEBUG",
                    f"replace_variables: 变量管理器状态 - 局部变量: {self.variable_manager.local_variables}",
                    step_index,
                )
                self._log_message(
                    "DEBUG",
                    f"replace_variables: 变量管理器状态 - 全局变量: {self.variable_manager.global_variables}",
                    step_index,
                )
                ...
                if var_name in self.variable_manager.local_variables:
                    ...
                    self._log_message(
                        "DEBUG",
                        f"replace_variables: 从局部变量获取 {var_name} = {var_value}",
                        step_index,
                    )
                # 尝试从全局变量中获取
                elif var_name in self.variable_manager.global_variables:
                    ...
                    self._log_message(
                        "DEBUG",
                        f"replace_variables: 从全局变量获取 {var_name} = {var_value}",
                        step_index,
                    )
            ...
            if not var_value and step_result:
                ...
                self._log_message(
                    "DEBUG",
                    f"replace_variables: 从步骤结果获取 {var_name} = {var_value}",
                    step_index,
                )
            ...
            self._log_message(
                "DEBUG",
                f"replace_variables: 变量替换: ${{{var_name}}} -> {var_value}",
                step_index,
            )
        ...
        # 调试：记录替换后的文本
        self._log_message(
            "DEBUG", f"replace_variables: 替换后文本 = {text}", step_index
        )
