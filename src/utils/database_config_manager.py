            ...
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if config_data.get('is_default'):
                        cursor.execute("UPDATE database_configs SET is_default = FALSE WHERE is_default = TRUE")
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    # 如果设置为默认配置，先取消其他配置的默认状态
                    if config_data.get('is_default'):
                        cursor.execute("UPDATE database_configs SET is_default = FALSE WHERE is_default = TRUE AND id != %s", (config_id,))
    ...
    def delete_config(self, config_id: int) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    # 检查是否为默认配置，如果是则不能删除
                    cursor.execute("SELECT is_default FROM database_configs WHERE id = %s", (config_id,))
    ...
    def test_connection(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                # 获取数据库大小
                cursor.execute(f"SELECT SUM(data_length + index_length) as size FROM information_schema.tables WHERE table_schema = '{config_data['database_name']}'")
                ...
                result['details'] = {
                    ...
                    'character_sets': {item['Variable_name']: item['Value'] for item in charset_info}
                }
        ...
        try:
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # 设置指定配置为默认
                    cursor.execute("UPDATE database_configs SET is_default = TRUE WHERE id = %s", (config_id,))
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                with conn.cursor() as cursor:
                    for config in configs:
                        # 检查配置是否已存在
                        cursor.execute("SELECT id FROM database_configs WHERE name = %s", (config['name'],))
...
if __name__ == "__main__":
    ...
    print("所有数据库配置:")
    for config in configs:
        print(f"  - {config['name']}: {config['host']}:{config['port']}/{config['database_name']}")
