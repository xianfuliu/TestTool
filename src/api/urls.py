...
def create_route_config(method: str, path: str, controller_method: Callable, summary: str = "", tags: List[str] = None) -> Dict[str, Any]:
    """创建路由配置"""
    return {
        "method": method,
        "path": path,
...
url_routes = [
    create_route_config("GET", "/health", health_controller.health_check, "健康检查接口", ["健康检查"]),
    create_route_config("GET", "/health/system", health_controller.system_info, "系统信息接口", ["健康检查", "系统信息"]),
    create_route_config("GET", "/system/status", health_controller.system_status, "系统状态接口", ["健康检查", "系统状态"]),
    create_route_config("POST", "/data/data-sync", DataSyncController().sync_data, "数据同步接口", ["数据同步"])
]
